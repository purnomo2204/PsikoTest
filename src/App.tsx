...
import { auth, db, signInWithGoogle, logout, signInAsGuest, loginWithEmailAndPassword, registerWithEmailAndPassword, resetPassword } from './firebase';
import { onAuthStateChanged, updateEmail } from 'firebase/auth';
import type { User } from 'firebase/auth';
import { doc, getDoc, setDoc, collection, onSnapshot, query, where, addDoc, serverTimestamp, updateDoc, deleteDoc, orderBy, getDocs, Timestamp } from 'firebase/firestore';
import { UserProfile, TestType, TestResult, ClassInfo, Question, StudentData, TeacherSettings, AppNotification, CounselingLog } from './types';
...
const handleDownloadPDF = (result: TestResult, teacherSettings: any) => {
    ...
    if (!isUmum) {
      ...
      currentY += 8;
      doc.setFontSize(14);
      const headerSchoolName = result.studentSchoolName || teacherSettings?.schoolName || "NAMA SEKOLAH ANDA DISINI";
      doc.text(headerSchoolName.toUpperCase(), 105, currentY, { align: 'center' });
      currentY += 6;
      doc.setFontSize(9);
...
export const handleDownloadDetailedReport = (student: any, allResults: TestResult[], teacherSettings: TeacherSettings | null) => {
  ...
  if (!isUmum) {
    ...
    currentY += 8;
    doc.setFontSize(14);
    const headerSchoolName = student.schoolName || teacherSettings?.schoolName || "NAMA SEKOLAH ANDA DISINI";
    doc.text(headerSchoolName.toUpperCase(), 105, currentY, { align: 'center' });
    currentY += 6;
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
  ...
  doc.setTextColor(30, 41, 59);
  doc.setFont("helvetica", "normal");
  doc.text(`${isUmum ? 'Nama Peserta' : 'Nama Siswa'}: ${student.name}`, 20, currentY);
  currentY += 7;
  doc.text(`${isUmum ? 'Nama Sekolah/ Alamat' : 'PASSWORD'}: ${isUmum ? (student.schoolName || student.password || '-') : (student.password || '-')}`, 20, currentY);
  currentY += 7;
  doc.text(`${isUmum ? 'Kelas/ Umur' : 'Kelas'}: ${student.className || (isUmum ? '-' : 'Peserta Umum')}`, 20, currentY);
  currentY += 7;
  ...
  targetTests.forEach(testType => {
    ...
    if (latestTest) {
      ...
      if (testType === 'subject_interest' && latestTest.extraData?.reasons) {
        const selected = Object.entries(latestTest.scores).filter(([_, v]) => v > 0);
        selected.forEach(([id, _]) => {
          const map: Record<string, string> = {
            agama: "Pendidikan Agama", ppkn: "PPKn", b_indo: "Bahasa Indonesia",
            mtk: "Matematika", ipa: "IPA", ips: "IPS", b_ing: "Bahasa Inggris",
  ...
  <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 gap-6 overflow-hidden relative">
    ...
    <div className="text-center space-y-3 z-10">
      <div className="space-y-1">
        <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase">PsikoTest</h2>
      ...
      <div className="flex flex-col items-center gap-2">
        <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-slate-200 shadow-sm animate-pulse">
          ...
          <span className="text-xs font-black text-slate-800 uppercase tracking-widest">Menyiapkan Dashboard</span>
        </div>
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-2">Dutatama Software Aplikasi</p>
      </div>
    </div>
  </div>
);
...
const PulseLoader = ({ text = "Sedang memproses..." }: { text?: string }) => (
  <div className="flex flex-col items-center justify-center p-12 space-y-6">
    ...
    <div className="text-center">
      <p className="text-sm font-black text-slate-800 uppercase tracking-widest animate-pulse mb-1">{text}</p>
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 leading-relaxed">Masukkan nomor Password_mu dengan benar, agar hasil tes tidak tertukar dengan siswa lain.</p>
            </div>
          </div>
        ...
        <div className="flex justify-between h-20 items-center">
          ...
          {user && (
            <div className="flex items-center gap-5">
                          ...
                          notifications.map((n) => (
                            <div 
                              ...
                              className={cn(
                                ...
                                !n.read && (n.type === 'warning' ? "bg-red-50" : "bg-emerald-50/30"),
                                n.type === 'warning' && "border-l-4 border-red-500"
                              )}
                            >
                              <div className={cn(
                                ...
                                {n.type === 'warning' ? <AlertTriangle className="w-5 h-5" /> : 
                                 n.type === 'info' ? <Info className="w-5 h-5" /> : <CheckCircle2 className="w-5 h-5" />}
                              </div>
                              <div className="flex-1">
                                <div className="flex justify-between items-start mb-0.5">
                                  <h5 className={cn("text-xs font-bold", n.type === 'warning' ? "text-red-900" : "text-slate-900")}>{n.title}</h5>
                                  {!n.read && <div className={cn("w-2 h-2 rounded-full", n.type === 'warning' ? "bg-red-600" : "bg-emerald-600")} />}
                                </div>
                                <p className={cn("text-[11px] leading-relaxed", n.type === 'warning' ? "text-red-700" : "text-slate-500")}>{n.message}</p>
              ...
              {(user.role === 'admin' || (user.role === 'teacher' && !(user.expiryDate && new Date(user.expiryDate.seconds * 1000) < new Date())) || user.email.toLowerCase() === "dutatama@gmail.com") && (
                <button 
                ...
                >
                  {isAdminView ? <BookOpen className="w-3.5 h-3.5 sm:w-4 h-4" /> : <LayoutDashboard className="w-3.5 h-3.5 sm:w-4 h-4" />}
                  <span className="hidden xs:inline">{isAdminView ? 'MENU SISWA' : 'MENU ADMIN'}</span>
                  <span className="xs:hidden">{isAdminView ? 'SISWA' : 'ADMIN'}</span>
                </button>
              )}
              {user.role === 'guest' && (
                <button 
                ...
                >
                  ...
                  <span className="hidden xs:inline">{view === 'history' ? 'BERANDA' : 'RIWAYAT SAYA'}</span>
                  <span className="xs:hidden">{view === 'history' ? 'BERANDA' : 'RIWAYAT'}</span>
                </button>
              )}
...
const TestCard = ({ test, type, onSelect, onPreview, recommendation, results }: { test: any, type: TestType, onSelect: (type: TestType) => void, onPreview: (type: TestType) => void, recommendation?: string, results: TestResult[] }) => {
      ...
      <div className="relative z-10 mt-auto pt-4 border-t border-slate-100">
        <div className="flex items-center justify-between gap-2">
          ...
          <button 
            ...
            className="px-3 py-2 border-2 border-slate-100 text-slate-600 rounded-lg font-bold text-xs hover:bg-slate-50 transition-all"
            title="Pratinjau Tes"
          >
...
const TestPreviewModal = ({ type, onClose, onStart }: { type: TestType, onClose: () => void, onStart: (type: TestType) => void }) => {
  const test = TESTS[type];
  if (!test) return null;

  const exampleQuestions = test.questions.slice(0, 3);
  const responseType = type === 'personality' || type === 'anxiety' ? 'Skala Likert (Sangat Tidak Setuju - Sangat Setuju)' : 'Pilihan Ganda';
      ...
      >
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-emerald-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-black text-slate-900 leading-none">Pratinjau: {test.title}</h3>
              <p className="text-slate-500 text-[10px] mt-1 font-bold uppercase tracking-wider">Metode: {responseType}</p>
            </div>
          </div>
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto custom-scrollbar space-y-8">
          <section>
            ...
            <p className="text-slate-600 text-sm leading-relaxed bg-slate-50 p-4 rounded-2xl border border-slate-100 font-medium">
              {test.description} Tes ini dirancang untuk mudah dipahami oleh siswa SMP maupun SMA. Pilih jawaban yang paling sesuai dengan diri kamu.
            </p>
          </section>
            ...
            <div className="space-y-3">
              {exampleQuestions.map((q: any, i: number) => (
                <div key={i} className="p-4 rounded-2xl border border-slate-100 bg-white shadow-sm">
                  <p className="text-sm font-bold text-slate-800 mb-3">{i + 1}. {q.text}</p>
                  <div className="space-y-2">
                    {type === 'personality' || type === 'anxiety' ? (
                      <div className="flex justify-between items-center px-4 py-3 bg-slate-50 rounded-xl border border-slate-100">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">Sangat Tidak Setuju</span>
                        <div className="flex gap-2.5">
                          {[1,2,3,4].map(n => (
                            <div key={n} className="w-7 h-7 rounded-full border-2 border-slate-200 bg-white" />
                          ))}
                        </div>
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">Sangat Setuju</span>
                      </div>
                    ) : (
                      q.options?.slice(0, 2).map((opt: any, idx: number) => (
                        <div key={idx} className="px-4 py-3 border border-slate-100 rounded-xl text-xs text-slate-600 font-bold flex items-center gap-3 bg-slate-50/50">
              ...
              <div className="text-center py-2">
                <p className="text-[10px] text-slate-400 font-bold italic uppercase tracking-widest">...dan {test.questions.length - 3} pertanyaan lainnya</p>
              </div>
            </div>
          </section>
        </div>
        ...
        <div className="p-6 bg-slate-50 flex flex-col sm:flex-row gap-3">
          <div className="flex-1 flex gap-2">
            <div className="flex-1 p-3 rounded-2xl bg-white border border-slate-200 text-center">
              <h5 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">ONLINE</h5>
              <p className="text-[10px] font-black text-emerald-600 uppercase">Tersedia</p>
            </div>
            <div className="flex-1 p-3 rounded-2xl bg-white border border-slate-200 text-center">
              <h5 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">OFFLINE</h5>
              <p className="text-[10px] font-black text-slate-400 uppercase">Tulis Tangan</p>
            </div>
          </div>
          <button 
          ...
          >
            MULAI TES SEKARANG <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </button>
        </div>
      </motion.div>
    </motion.div>
...
const getProgressKey = (uid: string, testType: string) => `${STORAGE_PREFIX}${uid}_${testType}`;

const ResumeTestModal = ({ testType, onResume, onStartFresh, onCancel }: { testType: TestType, onResume: () => void, onStartFresh: () => void, onCancel: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      ...
      >
        ...
        <h3 className="text-xl font-black text-slate-900 mb-2">Lanjutkan Tes?</h3>
        <p className="text-slate-500 text-sm mb-8 leading-relaxed">
          Kami menemukan progres pengerjaan tes <b>{testInfo?.title || 'ini'}</b> yang belum selesai. Apakah Anda ingin melanjutkannya?
        </p>
        ...
        <div className="space-y-3">
          ...
          >
            LANJUTKAN TES <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
          </button>
          <button 
...
const SubjectInterestTest = ({ onComplete, userId, onCancel }: { onComplete: (scores: Record<string, number>, extraData?: any) => void, userId: string, onCancel: () => void }) => {
  const subjects = [
  ...
  useEffect(() => {
    const saved = localStorage.getItem(getProgressKey(userId, 'subject_interest'));
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (selectedSubjects.length > 0) {
      localStorage.setItem(getProgressKey(userId, 'subject_interest'), JSON.stringify({
        selectedSubjects,
        reasons,
        timestamp: Date.now()
      }));
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center space-y-2">
        <h3 className="text-2xl font-black text-slate-900">Minat Terhadap Mata Pelajaran</h3>
        <p className="text-slate-500 text-sm">Pelajaran apa yang Anda sukai? Pilih mata pelajaran dan sebutkan alasannya.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Left Column: Subjects */}
        <div className="space-y-4">
          <h4 className="text-xs font-black text-emerald-600 uppercase tracking-widest border-b border-emerald-100 pb-2">Mata Pelajaran</h4>
            ...
            {subjects.map(subject => (
              ...
              >
                ...
                <div className={`w-5 h-5 rounded-md border-2 mr-3 flex items-center justify-center transition-all ${
                  selectedSubjects.includes(subject.id) ? 'bg-emerald-500 border-emerald-500' : 'bg-white border-slate-300'
                }`}>
                  {selectedSubjects.includes(subject.id) && <Check className="w-3.5 h-3.5 text-white" />}
                </div>
        ...
        <div className="space-y-4">
          <h4 className="text-xs font-black text-emerald-600 uppercase tracking-widest border-b border-emerald-100 pb-2">Alasan</h4>
          <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            {selectedSubjects.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-400 py-12">
                <Info className="w-8 h-8 mb-2 opacity-20" />
                <p className="text-xs font-medium text-center">Pilih mata pelajaran di sebelah kiri untuk memberikan alasan.</p>
              </div>
            ) : (
              selectedSubjects.map(id => {
                const subject = subjects.find(s => s.id === id);
                return (
                  <div key={id} className="space-y-2 animate-in fade-in slide-in-from-right-4 duration-300">
                    <label className="text-[10px] font-black text-slate-500 uppercase">{subject?.name}</label>
                    <textarea 
                      value={reasons[id] || ''}
...
const WarteggTest = ({ onComplete, userId, onCancel }: { onComplete: (scores: Record<string, number>, extraData?: any) => void, userId: string, onCancel: () => void }) => {
  ...
  useEffect(() => {
    if (Object.keys(drawings).length > 0 || Object.keys(titles).length > 0) {
      localStorage.setItem(getProgressKey(userId, 'wartegg'), JSON.stringify({
        drawings,
        titles,
        step,
        currentBox,
...
const TestForm = ({ type, userId, customTest, onComplete, onCancel, isAnalyzingTest }: { type: TestType, userId: string, customTest?: any, onComplete: (scores: Record<string, number>, extraData?: any) => void, onCancel: () => void, isAnalyzingTest?: boolean }) => {
  ...
  useEffect(() => {
    ...
    if (type !== 'wartegg' && type !== 'subject_interest') {
      ...
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          if (parsed.currentIdx > 0 || Object.keys(parsed.answers || {}).length > 0) {
            setPendingProgress(parsed);
            setShowResumeModal(true);
          }
        } catch (e) {
  ...
  useEffect(() => {
    if (type !== 'wartegg' && type !== 'subject_interest') {
      if (currentIdx > 0 || Object.keys(answers).length > 0) {
        localStorage.setItem(getProgressKey(userId, type), JSON.stringify({
          currentIdx,
          answers,
          scores,
          history,
    ...
    <div className="max-w-xl mx-auto py-6 px-4">
      ...
      <div className={`rounded-2xl shadow-lg border border-slate-100 overflow-hidden ${color}`}>
        <div className="p-6 sm:p-8">
          {type === 'wartegg' ? (
            <WarteggTest onComplete={onComplete} userId={userId} onCancel={onCancel} />
          ) : type === 'subject_interest' ? (
            <SubjectInterestTest onComplete={onComplete} userId={userId} onCancel={onCancel} />
          ) : (
            <>
...
const ResultView = ({ result, onBack, showToast, teacherSettings, results }: { result: TestResult, onBack: () => void, showToast: (m: string, t?: 'success' | 'error' | 'info') => void, teacherSettings: TeacherSettings | null, results: TestResult[] }) => {
  const [aiExplanation, setAiExplanation] = useState(result.aiExplanation || '');
  const [isGeneratingAI, setIsGeneratingAI] = useState(false);
  const [vType, setVType] = useState<'bar' | 'pie' | 'radar'>(result.visualizationType || 'bar');
  ...
  const chartData = Object.entries(result.scores).map(([name, value]) => {
    ...
    return { 
      name: indonesianMap[name] || name.charAt(0).toUpperCase() + name.slice(1).replace('_', ' '), 
      value 
    };
  });
  ...
  const renderChart = () => {
    ...
    if (result.testType === 'anxiety' || result.testType === 'cfit') {
      ...
      const rawScore = isAnxiety ? (result.scores['anxiety'] || 0) : isCfit ? (result.scores['cfit'] || 0) : Object.values(result.scores).reduce((a, b) => a + b, 0);
      const iqValue = isAnxiety ? rawScore : isCfit ? Math.round((rawScore / 40) * 70 + 70) : Math.round((rawScore / 100) * 60 + 80);
      const maxVal = isAnxiety ? 90 : 140; 
      const gLabel = isAnxiety ? "Skor Kecemasan" : isCfit ? "Estimasi IQ CFIT" : "Estimasi IQ";
      const gData = [
        { name: 'Value', value: iqValue, fill: isAnxiety ? (iqValue > 45 ? '#ef4444' : iqValue > 15 ? '#f59e0b' : '#10b981') : (iqValue >= 120 ? '#10b981' : iqValue >= 90 ? '#3b82f6' : '#f59e0b') },
        { name: 'Remaining', value: Math.max(0, maxVal - iqValue), fill: '#f1f5f9' }
      ];
      ...
      return (
        ...
        >
                  ...
                  if (active && payload && payload.length) {
                    return (
                      <div className="bg-white p-2 shadow-lg border border-slate-100 rounded-lg text-xs font-bold">
                        {payload[0].name === 'Value' ? `${gLabel}: ${payload[0].value}` : 'Sisa Skala'}
                      </div>
                    );
                  }
                  return null;
          ...
          <div className="text-center -mt-20">
            ...
            {isAnxiety && <span className="text-slate-400 font-bold ml-1 text-lg">/ {maxVal}</span>}
            <p className="text-xs font-black text-slate-400 uppercase tracking-[0.3em] mt-2 leading-none">{gLabel}</p>
            <motion.p 
              ...
              className={cn(
                ...
                isAnxiety 
                  ? (iqValue > 45 ? "bg-red-50 text-red-600 border border-red-100" : iqValue > 15 ? "bg-amber-50 text-amber-600 border border-amber-100" : "bg-emerald-50 text-emerald-600 border border-emerald-100")
                  : (iqValue >= 130 ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : iqValue >= 120 ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : iqValue >= 110 ? "bg-blue-50 text-blue-600 border border-blue-100" : iqValue >= 90 ? "bg-blue-50 text-blue-600 border border-blue-100" : "bg-amber-50 text-amber-600 border border-amber-100")
              )}
            >
              {isAnxiety 
                ? (iqValue > 45 ? "Tinggi" : iqValue > 15 ? "Sedang" : "Rendah")
                : (iqValue >= 130 ? "Sangat Superior" : iqValue >= 120 ? "Superior" : iqValue >= 110 ? "Rata-rata Atas" : iqValue >= 90 ? "Rata-rata" : "Bawah Rata-rata")}
            </motion.p>
          </div>
        </motion.div>
      );
          ...
          {[...Array(8)].map((_, i) => (
            ...
            >
              <div className="w-full aspect-square bg-slate-50 rounded-xl border border-slate-100 overflow-hidden mb-3 relative group">
                {drawings[i] ? (
                  <img src={drawings[i]} className="w-full h-full object-contain transition-transform duration-500 group-hover:scale-110" referrerPolicy="no-referrer" alt={`Wartegg ${i+1}`} />
                ) : (
              ...
              <p className="text-[11px] font-black text-slate-800 text-center line-clamp-1 h-4">{titles[i] || 'Tanpa Judul'}</p>
            </motion.div>
          ))}
        </div>
      );
    ...
    if (result.testType === 'subject_interest') {
      return (
        <div className="space-y-4">
          <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Mata Pelajaran yang Disukai:</h4>
          <div className="grid grid-cols-1 gap-3">
            {Object.entries(result.scores).filter(([_, v]) => v > 0).map(([id, _], index) => {
              const map: Record<string, string> = {
                agama: "Pendidikan Agama", ppkn: "PPKn", b_indo: "Bahasa Indonesia",
                mtk: "Matematika", ipa: "IPA", ips: "IPS", b_ing: "Bahasa Inggris",
                seni: "Seni Budaya", pjok: "PJOK", prakarya: "Prakarya",
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="font-black text-slate-800 text-sm uppercase tracking-tight">{map[id] || id}</span>
                  </div>
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="70%" data={chartData}>
                ...
                <PolarAngleAxis dataKey="name" fontSize={10} tick={{ fill: '#64748b', fontWeight: 'bold' }} />
                <PolarRadiusAxis fontSize={10} angle={30} tick={{ fill: '#94a3b8' }} />
                <RechartsRadar
                  name="Skor"
                  dataKey="value"
                  stroke="#10b981"
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} layout="vertical" margin={{ left: 20 }}>
                ...
                <Bar 
                  ...
                  fill="#10b981" 
                  radius={[0, 10, 10, 0]} 
                  label={{ position: 'right', fontSize: 10, fontWeight: 'bold', fill: '#1e293b' }} 
                  animationDuration={1500}
                  animationEasing="ease-out"
                />
              </BarChart>
  ...
  const handleAIExplanation = async () => {
    ...
    try {
      ...
      if (result.testType === 'wartegg' && result.extraData?.drawings) {
        ...
        const textPart = { text: `Sebagai seorang psikolog proyektif, tolong berikan analisa hasil tes Wartegg untuk siswa bernama ${result.studentName}.
        
Terdapat 8 kotak stimulus yang telah diselesaikan oleh siswa dengan judul sebagai berikut:
${Object.entries(result.extraData.titles || {}).sort(([a], [b]) => parseInt(a) - parseInt(b)).map(([k, v]) => `- Kotak ${parseInt(k) + 1}: ${v}`).join('\n')}
...
Gunakan bahasa Indonesia yang ramah, profesional, memotivasi, dan format Markdown yang rapi.` };
        ...
        const imageParts = Object.keys(result.extraData.drawings).sort((a, b) => parseInt(a) - parseInt(b)).map(key => {
          const drawingData = result.extraData.drawings[key];
          if (!drawingData) return null;
          const base64Data = drawingData.includes(',') ? drawingData.split(',')[1] : drawingData;
...
Gunakan bahasa Indonesia yang ramah, profesional, memotivasi, dan format Markdown yang rapi.`;
      ...
      showToast(error instanceof Error ? error.message : "Gagal mendapatkan analisa AI. Silakan coba lagi.", 'error');
    } finally {
      ...
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-3xl font-black mb-1 uppercase tracking-tight">Tes Selesai!</h2>
            <p className="text-sm text-emerald-100 font-medium opacity-80">Berikut adalah hasil analisis psikologi personal Anda.</p>
          </div>
        </motion.div>
        ...
        <div className="p-6 sm:p-10 space-y-10">
          ...
          >
            <div>
              <p className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-2 leading-none">Nama Siswa</p>
              <h4 className="text-base font-black text-slate-900 truncate">{result.studentName}</h4>
            </div>
            <div className="relative pl-6 before:absolute before:left-0 before:top-0 before:bottom-0 before:w-px before:bg-slate-200">
              <p className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-2 leading-none">Identitas / Sekolah</p>
              <h4 className="text-base font-black text-slate-900 truncate">{result.studentPassword || '-'}</h4>
            </div>
            <div className="relative pl-6 before:absolute before:left-0 before:top-0 before:bottom-0 before:w-px before:bg-slate-200">
              <p className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-2 leading-none">Kelas / Jenjang</p>
              <h4 className="text-base font-black text-slate-900 truncate">{result.studentClass || 'Umum'}</h4>
            </div>
          </motion.section>
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-6">
              ...
              {(!['anxiety', 'wartegg', 'subject_interest', 'cfit'].includes(result.testType)) && (
                <div className="flex bg-slate-100 p-1 rounded-xl shadow-inner">
                  <button 
                    onClick={() => setVType('bar')}
                    className={cn(
                      "p-2.5 rounded-lg transition-all flex items-center gap-2", 
                      vType === 'bar' ? "bg-white text-emerald-600 shadow-md ring-1 ring-slate-200/50" : "text-slate-400 hover:text-slate-600"
                    )}
                    title="Diagram Batang"
                  >
                    <BarChart3 className="w-4 h-4" />
                    {vType === 'bar' && <span className="text-[10px] font-black uppercase tracking-widest px-1">Bar</span>}
                  </button>
                  <button 
                    onClick={() => setVType('pie')}
                    className={cn(
                      "p-2.5 rounded-lg transition-all flex items-center gap-2", 
                      vType === 'pie' ? "bg-white text-emerald-600 shadow-md ring-1 ring-slate-200/50" : "text-slate-400 hover:text-slate-600"
                    )}
                    title="Diagram Pai"
                  >
                    <PieChartIcon className="w-4 h-4" />
                    {vType === 'pie' && <span className="text-[10px] font-black uppercase tracking-widest px-1">Pie</span>}
                  </button>
                  <button 
                    onClick={() => setVType('radar')}
                    className={cn(
                      "p-2.5 rounded-lg transition-all flex items-center gap-2", 
                      vType === 'radar' ? "bg-white text-emerald-600 shadow-md ring-1 ring-slate-200/50" : "text-slate-400 hover:text-slate-600"
                    )}
                    title="Diagram Radar"
                  >
                    <RadarIcon className="w-4 h-4" />
                    {vType === 'radar' && <span className="text-[10px] font-black uppercase tracking-widest px-1">Radar</span>}
                  </button>
                </div>
              )}
            </div>
          ...
          >
            <div className="absolute top-0 left-10 -translate-y-1/2 flex gap-2">
              <div className="px-4 py-1.5 bg-emerald-600 text-white rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-lg shadow-emerald-200/50">Analisis Hasil</div>
            </div>

            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 my-4">
              <div dangerouslySetInnerHTML={{ __html: summaryText }} className="text-sm text-slate-600 leading-relaxed max-w-lg" />
              <div className="flex items-center gap-3">
                <button 
          ...
          <AnimatePresence mode="wait">
            {aiExplanation ? (
              ...
              >
                <div className="flex items-center justify-between border-b border-indigo-100 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-black text-indigo-900 leading-none">Interpretasi AI</h3>
                      <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mt-1">Advanced Psychology Analysis</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => handleDownloadDetailedReport(result, results, teacherSettings)}
              ...
              >
                ...
                <div className="space-y-2">
                  <h4 className="text-xl font-black text-slate-800">Butuh Analisis Mendalam?</h4>
                  <p className="text-slate-500 text-sm max-w-xs mx-auto">Gunakan kecerdasan buatan untuk merangkum dan memberikan saran personal berdasarkan hasil tes Anda.</p>
                </div>
                <button 
                  ...
                  className={cn(
                    ...
                    isGeneratingAI && "opacity-75 cursor-not-allowed bg-emerald-500"
                  )}
                >
                  {isGeneratingAI ? (
                    <>
...
const TestCreator = ({ onBack, showToast, user }: { onBack: () => void, showToast: (m: string, t?: 'success' | 'error' | 'info') => void, user: UserProfile }) => {
...
const GuestRecap = ({ results, teacherSettings, classes, onEdit, onDelete }: { 
  ...
  const handleDownloadPDF = () => {
    ...
    doc.setFontSize(16);
    const headerSchoolName = guestSummary[0]?.tests[0]?.studentSchoolName || teacherSettings?.schoolName || "NAMA SEKOLAH ANDA DISINI";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden overflow-x-auto">
        <div className="overflow-x-auto min-w-[1000px]" style={{ transform: 'rotateX(180deg)' }}>
            ...
            <tbody className="divide-y divide-slate-100">
              {guestSummary.map((g, idx) => (
                <tr key={idx} className="hover:bg-slate-50/50 transition-colors border-b border-slate-200">
                  ...
                  <td className="px-6 py-4 border-r border-slate-200">
                    <span className="text-xs font-medium text-slate-600">{g.tests[0]?.studentSchoolName || '-'}</span>
                  </td>
                  <td className="px-6 py-4 border-r border-slate-200">
                    <div className="flex flex-col gap-1">
                      {g.tests.length > 0 ? (
                        g.tests.map((t, i) => (
                          <span key={i} className="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded text-[9px] font-black border border-indigo-100 uppercase w-fit">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      {g.tests.length > 0 ? (
                        g.tests.map((t, i) => (
                          <div key={i} className="text-xs font-bold text-indigo-700 bg-indigo-50 px-2 py-1 rounded-lg border border-indigo-100 w-fit">
...
const TestRecap = ({ results, classes, students, teacherSettings, onEdit, onDelete }: { 
  ...
  const studentResults = allStudents.filter(s => selectedClass === 'all' || s.className === selectedClass).map(student => {
    ...
    const schoolCompare = (a.schoolName || '').localeCompare(b.schoolName || '', undefined, { sensitivity: 'base' });
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text(teacherSettings?.dinasName?.toUpperCase() || "DINAS PENDIDIKAN", 148.5, 22, { align: 'center' });
    doc.setFontSize(16);
    const headerSchoolName = studentResults[0]?.tests[0]?.studentSchoolName || teacherSettings?.schoolName || "NAMA SEKOLAH ANDA DISINI";
    doc.text(headerSchoolName.toUpperCase(), 148.5, 30, { align: 'center' });
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(teacherSettings?.schoolAddress || "Alamat Lengkap Sekolah, No. Telp, Website, Email", 148.5, 36, { align: 'center' });
  ...
  const handleExportExcel = () => {
    const data = studentResults.map((s, idx) => {
      ...
      Object.keys(TESTS).forEach(type => {
        ...
        row[TESTS[type as TestType].title] = test ? getShortResult(type as TestType, test.scores) : '-';
      });
    ...
    XLSX.writeFile(wb, `rekap_hasil_tes_${selectedClass}_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden overflow-x-auto">
        <div className="overflow-x-auto min-w-[800px]" style={{ transform: 'rotateX(180deg)' }}>
          <div style={{ transform: 'rotateX(180deg)' }}>
            ...
            <tbody className="divide-y divide-slate-100">
              {studentResults.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((s, idx) => (
                <tr key={s.id || idx} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4 text-xs text-slate-500">{(currentPage - 1) * itemsPerPage + idx + 1}</td>
                  <td className="px-6 py-4 text-xs font-mono text-slate-400">{s.number}</td>
                  <td className="px-6 py-4 text-xs font-mono text-slate-400">{s.password || '-'}</td>
                  <td className="px-6 py-4">
...
const ManajemenTamu = ({ users, classes, setEditingStudent, setConfirmDelete, selectedGuestIds, setSelectedGuestIds }: { 
  users: any[], 
  classes: ClassInfo[],
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          ...
          <div className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl flex items-center gap-2">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs font-bold text-indigo-600 leading-none mt-0.5">{filteredGuests.length}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div style={{ transform: 'rotateX(180deg)' }}>
            ...
            <thead className="bg-slate-50 text-slate-500 text-[10px] uppercase tracking-widest font-bold">
              <tr>
                ...
                <th className="px-6 py-4 whitespace-nowrap text-center">Jenjang</th>
                <th className="px-6 py-4 whitespace-nowrap">Nama Sekolah/ Alamat</th>
                <th className="px-6 py-4 whitespace-nowrap text-center">Kelas/ Umur</th>
                <th className="px-6 py-4 whitespace-nowrap text-right">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {filteredGuests.length > 0 ? (
                filteredGuests.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((g, idx) => (
                  <tr key={g.uid || idx} className={`hover:bg-slate-50/50 transition-colors ${selectedGuestIds.includes(g.uid) ? 'bg-indigo-50/30' : ''}`}>
                    ...
                    <td className="px-6 py-4 text-xs text-slate-500 whitespace-nowrap text-center font-bold">{(currentPage - 1) * itemsPerPage + idx + 1}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <span className="text-xs font-black text-slate-700">{g.className || '-'}</span>
                    </td>
...
const MonitorSiswa = ({ results, students, classes, teacherSettings, setTestResult, setEditingStudent, setConfirmDelete }: { 
  results: TestResult[], 
  students: StudentData[], 
  classes: ClassInfo[],
  teacherSettings: TeacherSettings | null,
  ...
  const filteredStudents = React.useMemo(() => {
    return students.filter(s => {
      ...
      const matchesSchool = selectedSchool === 'all' || (s.schoolName && s.schoolName.toLowerCase() === selectedSchool.toLowerCase());
      const matchesSearch = s.name.toLowerCase().includes(search.toLowerCase()) || 
                           (s.password && s.password.toLowerCase().includes(search.toLowerCase()));
      
      // Fast lookup instead of results.filter inside map
      const studentResults = resultsByStudent[s.id!] || resultsByStudent[`${s.name}_${s.className}`] || [];
  ...
  }, [students, resultsByStudent, selectedClass, selectedSchool, search, statusFilter]);
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            ...
            <div className="relative flex-1 sm:flex-none">
              ...
              >
                ...
                {[...new Set(students.map(s => s.schoolName).filter(Boolean))].sort().map(school => (
          ...
          <div style={{ transform: 'rotateX(180deg)' }}>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredStudents.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((s, idx) => {
                ...
                return (
                  <tr key={s.id || idx} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 text-xs text-slate-500 whitespace-nowrap">{(currentPage - 1) * itemsPerPage + idx + 1}</td>
                    <td className="px-6 py-4 text-xs font-mono text-slate-400 whitespace-nowrap">{s.password || '-'}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm font-bold text-slate-900">{s.name}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-slate-600">{s.schoolName || 'Belum diatur'}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
...
const LaporanIndividuSiswa = ({ results, classes, students, teacherSettings, onEdit, onDelete }: { 
  results: TestResult[], 
  classes: ClassInfo[], 
  students: StudentData[], 
  teacherSettings: TeacherSettings | null,
  ...
  const studentResults = students.map(student => {
    const hasTests = results.some(r => r.studentId === student.id || (r.studentName === student.name && r.studentClass === student.className));
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div>
          <h3 className="text-xl font-bold text-slate-900 uppercase tracking-tight">Laporan Individu Siswa</h3>
          <p className="text-sm text-slate-500 font-medium">Cetak laporan hasil tes psikologi untuk masing-masing siswa.</p>
        </div>
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead className="bg-slate-50 text-slate-500 text-[10px] uppercase tracking-widest font-black border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 text-center text-indigo-600">JENIS TES</th>
                <th className="px-6 py-4 text-right">AKSI</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {studentResults.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((s, idx) => (
                <tr key={s.id || idx} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4 text-sm text-slate-500 text-center font-bold">{(currentPage - 1) * itemsPerPage + idx + 1}</td>
                  <td className="px-6 py-4">
                    <span className="text-sm font-black text-slate-900">{s.name}</span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <span className="px-2.5 py-1 bg-slate-100 text-slate-600 rounded-lg text-[10px] font-black uppercase tracking-wider">{s.className}</span>
                  </td>
                  <td className="px-6 py-4 text-xs text-slate-600 font-bold">{s.schoolName || '-'}</td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button 
...
const LaporanIndividuUmum = ({ results, classes, users, teacherSettings, onDelete, selectedGuestIds, setSelectedGuestIds, setConfirmDelete }: { 
  results: TestResult[], 
  classes: ClassInfo[], 
  users: any[], 
  teacherSettings: TeacherSettings | null,
  ...
  const allGuestUsers = users.filter(u => 
    u.role === 'student' && 
    ((u.className || '').toLowerCase().trim() === 'umum' || !registeredClassNames.includes((u.className || '').trim()))
  )
    .map(guest => {
      const hasTests = results.some(r => r.studentId === guest.uid || (r.studentName === guest.name && r.studentClass === guest.className));
      return { ...guest, hasTests };
    })
    .filter(g => g.hasTests);

  const uniqueSchools = [...new Set(allGuestUsers.map(g => g.schoolName).filter(Boolean))].sort();
  const uniqueClasses = [...new Set(allGuestUsers.map(g => g.className).filter(Boolean))].sort();
        ...
        <div className="flex items-center gap-4">
          <button 
            onClick={() => setConfirmDelete({
              ...
              message: 'Apakah Anda yakin ingin menghapus SEMUA data laporan individu peserta umum beserta riwayat tesnya? Tindakan ini tidak dapat dibatalkan.'
            })}
          ...
          <div>
            <h3 className="text-xl font-bold text-slate-900 uppercase tracking-tight">Laporan Individu Umum</h3>
            <p className="text-sm text-slate-500 font-medium tracking-tight">Cetak laporan hasil tes psikologi untuk masing-masing peserta umum.</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          {selectedGuestIds.length > 0 && (
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden overflow-x-auto">
        <div className="overflow-x-auto min-w-[1000px]">
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead className="bg-slate-50 text-slate-500 text-[10px] uppercase tracking-widest font-black border-b border-slate-200">
              <tr>
                <th className="px-6 py-4 text-center">
                  <input 
                    type="checkbox" 
                    checked={filteredUsers.length > 0 && selectedGuestIds.length === filteredUsers.length}
                    onChange={toggleSelectAll}
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredUsers.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((g, idx) => (
                <tr key={g.uid || idx} className={`hover:bg-slate-50/50 transition-colors ${selectedGuestIds.includes(g.uid) ? 'bg-emerald-50/30' : ''}`}>
                  <td className="px-6 py-4 text-center">
                    <input 
                      type="checkbox" 
                      checked={selectedGuestIds.includes(g.uid)}
                  ...
                  <td className="px-6 py-4 text-xs text-slate-600 font-bold whitespace-nowrap overflow-hidden text-ellipsis max-w-[200px]">{g.schoolName || '-'}</td>
                  <td className="px-6 py-4 text-center">
                    <div className="flex flex-wrap gap-1 justify-center max-w-[200px]">
                      {results.filter(r => r.studentId === g.uid || (r.studentName === g.name && r.studentClass === g.className)).map((r, i) => (
                        <span key={i} className="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded text-[8px] font-bold border border-indigo-100">
                          {TESTS[r.testType]?.title.replace('Tes ', '') || r.testType}
                        </span>
                      ))}
                      {results.filter(r => r.studentId === g.uid || (r.studentName === g.name && r.studentClass === g.className)).length === 0 && <span className="text-slate-400 italic text-[10px]">-</span>}
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
...
const HasilTesSummary = ({ results, classes, students, teacherSettings, onEdit, onDelete }: { 
  ...
  const [selectedClass, setSelectedClass] = useState<string>('all');
  const [selectedSchool, setSelectedSchool] = useState<string>('all');
  const [sortConfig, setSortConfig] = useState<{ key: 'schoolName' | 'name' | 'className', direction: 'asc' | 'desc' } | null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;
  ...
  const studentResults = React.useMemo(() => {
    return students.filter(s => {
      ...
      const studentTests = resultsByStudent[student.id!] || resultsByStudent[`${student.name}_${student.className}`] || [];
      ...
      if (sortConfig && sortConfig.key === 'schoolName') {
        ...
        if (sortConfig.direction === 'asc') return schoolA.localeCompare(schoolB);
  ...
  }, [students, resultsByStudent, selectedClass, selectedSchool, testTypes, sortConfig]);
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text(teacherSettings?.dinasName?.toUpperCase() || "DINAS PENDIDIKAN", 148.5, 22, { align: 'center' });
    doc.setFontSize(16);
    const headerSchoolName = studentResults[0]?.schoolName || teacherSettings?.schoolName || "NAMA SEKOLAH ANDA DISINI";
    doc.text(headerSchoolName.toUpperCase(), 148.5, 30, { align: 'center' });
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(teacherSettings?.schoolAddress || "Alamat Lengkap Sekolah, No. Telp, Website, Email", 148.5, 36, { align: 'center' });
    ...
    autoTable(doc, {
      ...
      head: [
        [
          ...
          { content: 'PASSWORD', rowSpan: 2, styles: { halign: 'center', valign: 'middle' } },
          ...
          <div style={{ transform: 'rotateX(180deg)' }}>
            ...
            <thead className="bg-slate-50 text-slate-500 text-[9px] uppercase tracking-tighter font-bold border-b border-slate-200">
              <tr>
                ...
                <th rowSpan={2} className="px-4 py-3 border-r border-slate-200 text-center align-middle">Password</th>
                ...
                <th rowSpan={2} className="px-4 py-3 border-r border-slate-200 text-center align-middle cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => {
                  setSortConfig(prev => ({
                    key: 'schoolName',
                    direction: prev?.key === 'schoolName' && prev.direction === 'asc' ? 'desc' : 'asc'
                  }));
                }}>
                ...
                <th colSpan={10} className="px-4 py-2 border-b border-slate-200 text-center">Jenis Tes</th>
              ...
              <tr>
                ...
                <th className="px-4 py-2 border-r border-slate-200 text-center">CFIT</th>
                <th className="px-4 py-2 border-r border-slate-200 text-center">Wartegg</th>
                <th className="px-4 py-2 border-r border-slate-200 text-center">Minat Mapel</th>
                <th className="px-4 py-2 border-r border-slate-200 text-center">Per. Karier</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {studentResults.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((s, idx) => (
                <tr key={s.id || idx} className="hover:bg-emerald-50/30 transition-colors">
                  <td className="px-2 py-1.5 text-[10px] text-slate-500 text-center border-r border-slate-100 italic">{(currentPage - 1) * itemsPerPage + idx + 1}</td>
                  <td className="px-2 py-1.5 text-[10px] font-mono text-slate-400 text-center border-r border-slate-100">{s.password || '-'}</td>
                  <td className="px-3 py-1.5 border-r border-slate-100">
                    <span className="text-xs font-bold text-slate-900 leading-tight block">{s.name}</span>
                  </td>
                  <td className="px-2 py-1.5 text-center border-r border-slate-100">
                    <span className="px-1.5 py-0.5 bg-slate-100 text-slate-600 rounded text-[9px] font-black uppercase tracking-tighter">{s.className}</span>
                  </td>
                  <td className="px-2 py-1.5 text-[10px] text-slate-600 text-center border-r border-slate-100 font-medium whitespace-nowrap overflow-hidden text-ellipsis max-w-[100px]">{s.schoolName || '-'}</td>
                  <td className="px-1.5 py-1.5 text-[9px] text-slate-600 text-center border-r border-slate-100 font-bold whitespace-nowrap">{s.latestTests['learning_style']}</td>
                  <td className="px-1.5 py-1.5 text-[9px] text-slate-600 text-center border-r border-slate-100 font-bold whitespace-nowrap">{s.latestTests['personality']}</td>
                  <td className="px-1.5 py-1.5 text-[9px] text-slate-600 text-center border-r border-slate-100 font-bold whitespace-nowrap">{s.latestTests['multiple_intelligences']}</td>
                  <td className="px-1.5 py-1.5 text-[9px] text-slate-600 text-center border-r border-slate-100 font-bold whitespace-nowrap">{s.latestTests['aptitude_interest']}</td>
                  <td className="px-1.5 py-1.5 text-[9px] text-slate-600 text-center border-r border-slate-100 font-bold whitespace-nowrap">{s.latestTests['school_major']}</td>
                  <td className="px-1.5 py-1.5 text-[9px] text-slate-600 text-center border-r border-slate-100 font-bold whitespace-nowrap">{s.latestTests['anxiety']}</td>
                  <td className="px-1.5 py-1.5 text-[9px] text-slate-600 text-center border-r border-slate-100 font-bold whitespace-nowrap">{s.latestTests['cfit']}</td>
                  <td className="px-1.5 py-1.5 text-[9px] text-slate-600 text-center border-r border-slate-100 font-bold whitespace-nowrap">{s.latestTests['wartegg']}</td>
                  <td className="px-1.5 py-1.5 text-[9px] text-slate-600 text-center border-r border-slate-100 font-bold whitespace-nowrap">{s.latestTests['subject_interest']}</td>
                  <td className="px-1.5 py-1.5 text-[9px] text-slate-600 text-center border-r border-slate-100 font-bold whitespace-nowrap">{s.latestTests['school_career']}</td>
...
const HasilTesTamu = ({ results, classes, users, teacherSettings, onDelete, setConfirmDelete, selectedGuestIds, setSelectedGuestIds }: { 
  results: TestResult[], 
  classes: ClassInfo[], 
  users: any[], 
  teacherSettings: TeacherSettings | null,
  ...
  const [sortConfig, setSortConfig] = useState<{ key: 'schoolName' | 'name', direction: 'asc' | 'desc' } | null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;
  ...
  const guestResults = guestUsers.map(guest => {
    ...
    const guestTests = results.filter(r => 
      r.studentId === guest.uid || 
      ((r.studentName || '').trim().toLowerCase() === guestNameLower && 
       (r.studentClass || '').trim().toLowerCase() === guestClassLower)
    );
    const guestSchoolName = guestTests.find(r => r.studentSchoolName)?.studentSchoolName || guest.schoolName || '-';
  ...
  .filter(g => {
    ...
    const matchesSchool = filterSchool === 'all' || g.schoolName === filterSchool;
    const matchesJenjang = filterJenjang === 'all' || g.jenjang === filterJenjang;
  ...
  const uniqueSchools = [...new Set(guestUsers.map(g => g.schoolName).filter(Boolean))].sort();
  const uniqueJenjangs = [...new Set(guestUsers.map(g => g.jenjang).filter(Boolean))].sort();
  const uniqueClasses = [...new Set(guestUsers.map(g => g.className).filter(Boolean))].sort();
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text(teacherSettings?.dinasName?.toUpperCase() || "DINAS PENDIDIKAN", 148.5, 22, { align: 'center' });
    doc.setFontSize(16);
    const headerSchoolName = guestResults[0]?.schoolName || teacherSettings?.schoolName || "NAMA SEKOLAH ANDA DISINI";
    doc.text(headerSchoolName.toUpperCase(), 148.5, 30, { align: 'center' });
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(teacherSettings?.schoolAddress || "Alamat Lengkap Sekolah, No. Telp, Website, Email", 148.5, 36, { align: 'center' });
    ...
    autoTable(doc, {
      ...
      head: [
        [
          ...
          { content: 'KELAS/ UMUR', rowSpan: 2, styles: { halign: 'center', valign: 'middle' } },
          { content: 'JENIS TES', colSpan: 10, styles: { halign: 'center' } }
        ],
        [
          { content: 'GAYA BELAJAR', styles: { halign: 'center' } },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div>
          <h3 className="text-xl font-bold text-slate-900 uppercase tracking-tight">Hasil Tes Umum</h3>
          <p className="text-sm text-slate-500 font-medium">Rangkuman hasil akhir seluruh tes psikologi peserta umum.</p>
        </div>
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          <div className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl flex items-center gap-2 min-w-[100px]">
            <Users className="w-3.5 h-3.5 text-slate-400" />
            <div className="flex flex-col">
              ...
              <span className="text-xs font-bold text-indigo-600 leading-none mt-0.5">{guestResults.length}</span>
            </div>
          </div>
          <button 
            onClick={() => setConfirmDelete({
              ...
              message: 'Apakah Anda yakin ingin menghapus SEMUA data peserta umum? Tindakan ini akan menghapus semua hasil tes dan riwayat terkait secara permanen. Tindakan ini tidak dapat dibatalkan.'
            })}
          ...
          {selectedGuestIds.length > 0 && (
            ...
            >
              <Trash2 className="w-4 h-4" /> HAPUS TERPILIH ({selectedGuestIds.length})
            </button>
          )}
          <button 
            onClick={handleDownloadPDF}
        ...
        <div className="overflow-x-auto" style={{ transform: 'rotateX(180deg)' }}>
          <div style={{ transform: 'rotateX(180deg)' }}>
            ...
            <thead className="bg-slate-50 text-slate-500 text-[9px] uppercase tracking-tighter font-black border-b border-slate-200">
              <tr>
                <th rowSpan={2} className="px-4 py-3 border-r border-slate-200 text-center align-middle whitespace-nowrap">
                  <input 
                    type="checkbox" 
                    onChange={handleSelectAll}
                    checked={guestResults.length > 0 && selectedGuestIds.length === guestResults.length}
                    className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"
                  />
                </th>
                <th rowSpan={2} className="px-4 py-3 border-r border-slate-200 text-center align-middle uppercase font-black text-[10px]">NO</th>
                <th rowSpan={2} className="px-4 py-3 border-r border-slate-200 text-center align-middle uppercase font-black text-[10px]">NAMA SISWA</th>
                <th rowSpan={2} className="px-4 py-3 border-r border-slate-200 text-center align-middle uppercase font-black text-[10px]">KELAS</th>
                <th rowSpan={2} className="px-4 py-3 border-r border-slate-200 text-center align-middle cursor-pointer hover:bg-slate-100 transition-colors uppercase font-black text-[10px]" onClick={() => {
                  setSortConfig(prev => ({
                    key: 'schoolName',
                    direction: prev?.key === 'schoolName' && prev.direction === 'asc' ? 'desc' : 'asc'
                  }));
                ...
                <th rowSpan={2} className="px-4 py-3 border-r border-slate-200 text-center align-middle uppercase font-black text-[10px]">KELAS/ UMUR</th>
                <th colSpan={10} className="px-4 py-2 border-b border-slate-200 text-center uppercase font-black text-[10px]">JENIS TES</th>
                <th rowSpan={2} className="px-4 py-3 text-center align-middle">Aksi</th>
              </tr>
              <tr>
                <th className="px-4 py-2 border-r border-slate-200 text-center">Gaya Belajar</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {guestResults.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((g, idx) => (
                <tr key={g.uid || idx} className={cn("hover:bg-indigo-50/30 transition-colors", selectedGuestIds.includes(g.uid) && "bg-indigo-50/50")}>
                  ...
                  <td className="px-2 py-1.5 text-[10px] text-slate-500 text-center border-r border-slate-100 font-bold tracking-tighter italic">{(currentPage - 1) * itemsPerPage + idx + 1}</td>
                  <td className="px-3 py-1.5 border-r border-slate-100">
                    <span className="text-xs font-bold text-slate-900 leading-tight block">{g.name || '-'}</span>
                  </td>
                  <td className="px-2 py-1.5 text-center border-r border-slate-100">
                  ...
                  <td className="px-2 py-1.5 text-center border-r border-slate-100">
                    <span className="text-[9px] font-black text-slate-700 px-1.5 py-0.5 bg-slate-100 rounded border border-slate-200 uppercase tracking-tighter">{g.className || '-'}</span>
                  </td>
              ...
              {guestResults.length === 0 && (
                <tr>
                  <td colSpan={16} className="p-12 text-center text-slate-400 font-bold italic">
                    Tidak ada data peserta umum ditemukan.
                  </td>
                </tr>
              )}
...
const CounselingDashboard = ({ students, counselingLogs, onOpenProfile }: { students: StudentData[], counselingLogs: CounselingLog[], onOpenProfile: (s: StudentData) => void }) => {
  const [search, setSearch] = useState('');
  ...
  }, [students, classFilter, studentNameFilter]);

  return (
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h3 className="text-xl font-bold text-slate-900">Catatan Konseling</h3>
          <p className="text-sm text-slate-500">Pantau dan kelola catatan bimbingan konseling siswa secara terpadu.</p>
        </div>
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredStudents.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((student, idx) => {
              ...
              const lastLog = [...studentLogs].sort((a, b) => (b.timestamp?.seconds || 0) - (a.timestamp?.seconds || 0))[0];
              
              return (
                <tr key={student.id} className="hover:bg-slate-50/80 transition-colors">
                  <td className="px-6 py-4 text-xs font-bold text-slate-400">{(currentPage - 1) * itemsPerPage + idx + 1}</td>
                  <td className="px-6 py-4 font-bold text-slate-900">{student.name}</td>
                  <td className="px-6 py-4 text-xs text-slate-600">{student.className}</td>
                  <td className="px-6 py-4 text-xs font-bold text-emerald-600">{studentLogs.length} Catatan</td>
                  <td className="px-6 py-4">
                    {lastLog ? (
                      <div className="flex flex-col">
                        <span className="text-xs text-slate-700 font-medium italic line-clamp-1">"{lastLog.topic}"</span>
                        <span className="text-[10px] text-slate-400">
                          {new Date(lastLog.timestamp?.seconds * 1000).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                        </span>
                      </div>
                    ) : (
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between items-center p-4 border-t border-slate-100 bg-slate-50/50 gap-4">
      <div className="text-xs font-bold text-slate-500">
        Menampilkan <span className="text-slate-900">{startItem}-{endItem}</span> dari <span className="text-slate-900">{totalItems}</span> data
      </div>
      <div className="flex items-center gap-2">
        <button 
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  currentPage === pageNum ? "bg-emerald-600 text-white shadow-md shadow-emerald-200" : "text-slate-600 hover:bg-slate-50"
                )}
              >
                {pageNum}
              </button>
...
const TAB_CONFIG: Record<string, { label: string, icon: any, colorClass?: string, activeColorClass?: string }> = {
  ...
  'report-individu-siswa': { label: 'Laporan Individu', icon: Printer, colorClass: "text-cyan-300 hover:bg-emerald-800/50 border border-emerald-700/50" },
  'report': { label: 'Laporan Kelas', icon: BarChart3 },
  'counseling': { label: 'Catatan Konseling', icon: MessageSquare },
  'manajemen-tamu': { label: 'Manajemen Umum', icon: Users, colorClass: "text-indigo-100/70 hover:bg-indigo-800/50", activeColorClass: "bg-indigo-600 text-white shadow-lg" },
  'tamu': { label: 'Riwayat Tes Umum', icon: History, colorClass: "text-indigo-100/70 hover:bg-indigo-800/50", activeColorClass: "bg-indigo-600 text-white shadow-lg" },
  'hasil-tes-tamu': { label: 'Hasil Tes Umum', icon: ClipboardCheck, colorClass: "text-indigo-100/70 hover:bg-indigo-800/50", activeColorClass: "bg-indigo-600 text-white shadow-lg" },
  'report-individu-umum': { label: 'Laporan Individu Umum', icon: Printer, colorClass: "text-indigo-300 hover:bg-indigo-800/50 border border-indigo-700/50", activeColorClass: "bg-indigo-600 text-white shadow-lg" },
  'teacher': { label: 'Pengaturan Guru', icon: UserCog },
  'customization': { label: 'KUSTOMISASI', icon: Settings2 },
  'admin-users': { label: 'ADMINISTRASI PENGGUNA', icon: ShieldCheck, activeColorClass: "bg-red-700 text-white shadow-lg", colorClass: "text-red-100/70 hover:bg-red-800/50" },
};
...
const AVAILABLE_TABS = [
  ...
  { id: 'report-individu-siswa', name: 'Laporan Individu', group: 'Manajemen Siswa' },
  ...
  { id: 'report-individu-umum', name: 'Laporan Individu Umum', group: 'Manajemen Umum' },
  { id: 'teacher', name: 'Pengaturan Guru', group: 'Administrasi' },
  { id: 'customization', name: 'Kustomisasi Dashboard', group: 'Administrasi' },
];

const AdminDashboard = ({ results: rawResults, classes, students, teacherSettings, user, setView, showToast, setTestResult, allUsers, notifications, counselingLogs, customTests }: { results: TestResult[], classes: ClassInfo[], students: StudentData[], teacherSettings: TeacherSettings | null, user: UserProfile, setView: (v: any) => void, showToast: (m: string, t?: 'success' | 'error' | 'info') => void, setTestResult: (r: TestResult | null) => void, allUsers: any[], notifications: AppNotification[], counselingLogs: CounselingLog[], customTests: any[] }) => {
  ...
  const results = React.useMemo(() => {
    ...
    rawResults.forEach(r => {
      const key = `${r.studentName}_${r.studentClass}_${r.testType}_${r.timestamp?.seconds || 0}`;
      if (!map.has(key)) {
        map.set(key, r);
      }
    });
  ...
  const [historySortField, setHistorySortField] = useState<'name' | 'class' | 'school' | 'type' | 'date'>('date');
  ...
  const [sortStudentField, setSortStudentField] = useState<'name' | 'className' | 'schoolName'>(() => (localStorage.getItem('adminSortStudentField') as any) || 'name');
  const [sortStudentOrder, setSortStudentOrder] = useState<'asc' | 'desc'>(() => (localStorage.getItem('adminSortStudentOrder') as any) || 'asc');
  ...
  const [tamuSortField, setTamuSortField] = useState<'name' | 'school' | 'type' | 'date'>('date');
  ...
  const handleBackupData = async () => {
    setIsBackingUp(true);
    try {
      const collectionsToBackup = ['students', 'test_results', 'counseling_logs', 'teacher_settings', 'classes', 'users', 'custom_tests', 'notifications'];
      ...
      showToast('Backup data berhasil diunduh. Simpan file ini di HDD atau Flashdisk Anda.', 'success');
    } catch (error) {
  ...
  const handleRestoreData = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!window.confirm("PERINGATAN: Proses restore akan menimpa/menambah data yang ada dengan data dari file backup. Lanjutkan?")) {
      event.target.value = '';
      return;
    }
    ...
    try {
      ...
      showToast(`Restore berhasil! ${totalImported} data telah dipulihkan.`, 'success');
    } catch (error) {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'history' | 'students' | 'teacher' | 'report' | 'recap' | 'tamu' | 'recap-tamu' | 'monitor' | 'hasil-tes' | 'manajemen-tamu' | 'hasil-tes-tamu' | 'counseling' | 'admin-users' | 'report-individu-siswa' | 'report-individu-umum' | 'customization'>(() => (localStorage.getItem('adminActiveTab') as any) || 'dashboard');
  
  const [enabledWidgets, setEnabledWidgets] = useState<string[]>(() => {
    const saved = localStorage.getItem('adminEnabledWidgets');
    if (saved) return JSON.parse(saved);
    return teacherSettings?.dashboardWidgets || AVAILABLE_WIDGETS.map(w => w.id);
  });
  ...
  useEffect(() => {
    if (teacherSettings?.dashboardWidgets) {
      ...
      localStorage.setItem('adminEnabledWidgets', JSON.stringify(teacherSettings.dashboardWidgets));
    }
    if (teacherSettings?.sidebarTabs) {
      setSidebarOrder(teacherSettings.sidebarTabs);
      localStorage.setItem('adminSidebarOrder', JSON.stringify(teacherSettings.sidebarTabs));
    }
  }, [teacherSettings]);
  ...
  const registeredClassNames = React.useMemo(() => classes.map(c => c.name), [classes]);
    ...
    return results
      ...
      .sort((a, b) => {
        ...
        if (tamuSortField === 'school') {
          ...
          cmp = schoolA.localeCompare(schoolB, undefined, { sensitivity: 'base' });
          ...
          cmp = (TESTS[a.testType]?.title || '').localeCompare(TESTS[b.testType]?.title || '', undefined, { sensitivity: 'base' });
        } else {
          // Default to date
          const timeA = a.timestamp?.seconds || 0;
          const timeB = b.timestamp?.seconds || 0;
  ...
  }, [results, registeredClassNames, tamuSearch, tamuTypeFilter, tamuDate, tamuSortField, tamuSortOrder]);
  ...
  const [confirmDelete, setConfirmDelete] = useState<{ id: string, type: 'student' | 'guest' | 'result' | 'all_students' | 'all_selected_students' | 'results_by_student' | 'results_by_guest' | 'results_by_selected_guests' | 'all_selected_results' | 'all_guests_comprehensive' | 'user' | 'all_selected_history_results', title: string, message: string, extraData?: any } | null>(null);
  ...
  const [viewingStudentProfile, setViewingStudentProfile] = useState<StudentData | null>(null);
  const [isAddingLog, setIsAddingLog] = useState(false);
  const [newLogTopic, setNewLogTopic] = useState('');
  const [newLogNotes, setNewLogNotes] = useState('');
  const [newLogStatus, setNewLogStatus] = useState<'planned' | 'ongoing' | 'completed'>('planned');
  ...
  const [studentClassFilter, setStudentClassFilter] = useState<string>(() => localStorage.getItem('adminStudentClassFilter') || 'all');
  const [studentStatusFilter, setStudentStatusFilter] = useState<string>(() => localStorage.getItem('adminStudentStatusFilter') || 'all');
  const [studentSchoolFilter, setStudentSchoolFilter] = useState<string>(() => localStorage.getItem('adminStudentSchoolFilter') || 'all');
  ...
  }, [selectedClass, filterType, tableClassFilter, tableTypeFilter, tamuTypeFilter, activeTab, studentStatusFilter, studentClassFilter, studentSchoolFilter]);
  ...
  const filteredStudents = students
    .filter(s => {
      ...
      const matchesSchool = studentSchoolFilter === 'all' || s.schoolName === studentSchoolFilter;
  ...
  const handleDeleteAllStudents = async () => {
    try {
      ...
      const allResultDocs = results.map(r => deleteDoc(doc(db, 'test_results', r.id!)));
      const allLogDocs = counselingLogs.map(l => deleteDoc(doc(db, 'counseling_logs', l.id!)));
      await Promise.all([...allResultDocs, ...allLogDocs]);
      
      setConfirmDelete(null);
      showToast('Semua data siswa dan data terkait berhasil dihapus.', 'success');
    } catch (error) {
  ...
  const handleDeleteSelectedStudents = async () => {
    ...
    try {
      const studentDocs = selectedStudentIds.map(id => deleteDoc(doc(db, 'students', id)));
      
      // Filter results and logs for these students
      const relatedResults = results.filter(r => selectedStudentIds.includes(r.studentId));
      const relatedLogs = counselingLogs.filter(l => selectedStudentIds.includes(l.studentId));
      
      const resultDocs = relatedResults.map(r => deleteDoc(doc(db, 'test_results', r.id!)));
      const logDocs = relatedLogs.map(l => deleteDoc(doc(db, 'counseling_logs', l.id!)));
      
      await Promise.all([...studentDocs, ...resultDocs, ...logDocs]);
      
      setSelectedStudentIds([]);
      setConfirmDelete(null);
      showToast(`${selectedStudentIds.length} Siswa dan semua data terkait berhasil dihapus.`, 'success');
    } catch (error) {
  ...
  const handleDeleteStudent = async (id: string) => {
    try {
      ...
      const resultDocs = studentResults.map(r => deleteDoc(doc(db, 'test_results', r.id!)));
      const logDocs = studentLogs.map(l => deleteDoc(doc(db, 'counseling_logs', l.id!)));
  ...
  const handleDeleteSelectedResults = async () => {
    ...
    try {
      const batch = selectedResultIds.map(id => deleteDoc(doc(db, 'test_results', id)));
      await Promise.all(batch);
      setSelectedResultIds([]);
      setConfirmDelete(null);
      showToast(`${selectedResultIds.length} Hasil tes berhasil dihapus.`, 'success');
    } catch (error) {
  ...
  const handleDeleteAllResultsForGuest = async (id: string, name: string) => {
    try {
      const relatedResults = results.filter(r => r.studentId === id || (r.studentName === name && r.studentClass.toLowerCase() === 'umum'));
      ...
      setConfirmDelete(null);
      showToast(`Data peserta ${name} dan semua riwayat berhasil dihapus.`, 'success');
    } catch (error) {
  ...
  const handleDeleteSelectedGuests = async () => {
    ...
    try {
      const userDocs = selectedGuestIds.map(id => deleteDoc(doc(db, 'users', id)));
      
      const relatedResults = results.filter(r => selectedGuestIds.includes(r.studentId));
      const relatedLogs = counselingLogs.filter(l => selectedGuestIds.includes(l.studentId));
      ...
      setSelectedGuestIds([]);
      setConfirmDelete(null);
      showToast(`${selectedGuestIds.length} Peserta umum dan semua data terkait berhasil dihapus.`, 'success');
    } catch (error) {
  ...
  const handleDeleteAllGuestsResults = async () => {
    const guestUsers = allUsers.filter(u => u.role === 'student' && !registeredClassNames.includes(u.className));
    const guestUserIds = guestUsers.map(u => u.uid);
    
    try {
      const userDocs = guestUserIds.map(id => deleteDoc(doc(db, 'users', id)));
      
      const relatedResults = results.filter(r => guestUserIds.includes(r.studentId) || r.studentClass.toLowerCase() === 'umum');
      const relatedLogs = counselingLogs.filter(l => guestUserIds.includes(l.studentId));
      ...
      setConfirmDelete(null);
      showToast('Semua data peserta umum dan riwayat terkait berhasil dihapus.', 'success');
    } catch (error) {
  ...
  const classAverages = React.useMemo(() => {
    ...
    const classGroups: Record<string, { totalScores: Record<string, number>, count: number }> = {};
    
    relevantResults.forEach(r => {
      if (!classGroups[r.studentClass]) {
        classGroups[r.studentClass] = { totalScores: {}, count: 0 };
      }
      
      Object.entries(r.scores).forEach(([scoreName, scoreValue]) => {
        classGroups[r.studentClass].totalScores[scoreName] = (classGroups[r.studentClass].totalScores[scoreName] || 0) + (scoreValue as number);
      });
    ...
    return Object.entries(classGroups).map(([className, data]) => ({
      ...
      averages: Object.fromEntries(
        Object.entries(data.totalScores).map(([scoreName, total]) => [scoreName, total / data.count])
      )
    }));
    ...
    return results
      ...
      .sort((a, b) => {
        ...
        if (historySortField === 'school') {
          const schoolA = a.studentSchoolName || students.find(s => s.id === a.studentId || (s.name === a.studentName && s.className === a.studentClass))?.schoolName || '';
          const schoolB = b.studentSchoolName || students.find(s => s.id === b.studentId || (s.name === b.studentName && s.className === b.studentClass))?.schoolName || '';
          cmp = schoolA.localeCompare(schoolB, undefined, { sensitivity: 'base' });
        } else if (historySortField === 'name') {
          cmp = (a.studentName || '').localeCompare(b.studentName || '', undefined, { sensitivity: 'base' });
        } else if (historySortField === 'class') {
          cmp = (a.studentClass || '').localeCompare(b.studentClass || '', undefined, { numeric: true, sensitivity: 'base' });
  ...
  }, [results, registeredClassNames, tableClassFilter, historySearch, tableTypeFilter, historyDate, historySortField, historySortOrder, students]);

  const totalPages = Math.ceil(tableFilteredResults.length / itemsPerPage);
  const paginatedResults = tableFilteredResults.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  const toggleHistorySort = (field: 'name' | 'class' | 'school' | 'type' | 'date') => {
  ...
  const handleDeleteSelectedHistoryResults = async () => {
    if (selectedHistoryIds.length === 0) return;
    try {
      const batch = selectedHistoryIds.map(id => deleteDoc(doc(db, 'test_results', id)));
      await Promise.all(batch);
      setSelectedHistoryIds([]);
      setConfirmDelete(null);
      setIsSelectingHistory(false);
      showToast(`${selectedHistoryIds.length} Hasil tes berhasil dihapus.`, 'success');
    } catch (error) {
      handleFirestoreError(error, OperationType.DELETE, 'multiple_history_results');
    }
  };
  ...
  const handleAddStudent = async () => {
    ...
    if (newStudentPassword) {
      if (!/^[0-9A-Z]+$/.test(newStudentPassword)) {
        showToast("Password hanya boleh berisi angka dan huruf besar.", "error");
        return;
      }
      
      const isDuplicate = students.some(s => s.password === newStudentPassword);
      if (isDuplicate) {
  ...
  const handleDownloadClassReport = () => {
    ...
    currentY += 8;
    doc.setFontSize(14);
    const headerSchoolName = reportResults.find(r => r.studentSchoolName)?.studentSchoolName || teacherSettings?.schoolName || "NAMA SEKOLAH ANDA DISINI";
    doc.text(headerSchoolName.toUpperCase(), 105, currentY, { align: 'center' });
    currentY += 6;
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
  ...
  const handlePrintLoginCards = (studentsToPrint: StudentData[]) => {
    if (studentsToPrint.length === 0) {
      showToast("Pilih siswa terlebih dahulu atau filter daftar siswa.", "info");
      return;
    }
    ...
    studentsToPrint.forEach((student) => {
      ...
      doc.text("KARTU LOGIN SISWA", x + cardWidth / 2, y + 7.5, { align: 'center' });
      ...
      doc.text("Gunakan data di atas untuk mengisi identitas pada aplikasi tes.", x + cardWidth / 2, y + cardHeight - 5, { align: 'center' });
      doc.setFontSize(5);
      doc.text("Dutatama Software Aplikasi", x + cardWidth / 2, y + cardHeight - 2, { align: 'center' });
    ...
    showToast(`${studentsToPrint.length} Kartu login berhasil dibuat.`, "success");
  };
  ...
  const handleDownloadTemplate = () => {
    ...
    showToast("Template berhasil diunduh. Silakan isi data sesuai format.", "success");
  };
      ...
      try {
        ...
        for (let i = 0; i < jsonData.length; i++) {
          ...
          const findValue = (searchKeys: string[]) => {
            ...
            const actualKey = Object.keys(row).find(k => lowerSearchKeys.includes(k.trim().toLowerCase()));
            return actualKey ? row[actualKey] : null;
          };

          const number = findValue(["No", "Nomor"]) || "";
          const password = findValue(["PASSWORD"]) ? String(findValue(["PASSWORD"])).trim() : "";
          const name = findValue(["Nama Siswa", "Nama", "Student Name"]);
          const className = findValue(["Kelas", "Class"]);
          const schoolName = findValue(["Nama Sekolah", "Sekolah", "Asal Sekolah", "School Name"]) || teacherSettings?.schoolName || "";
          ...
          if (password) {
            if (!/^\d+$/.test(password)) {
              errors.push(`Baris ${rowNum}: PASSWORD (${password}) harus berupa angka.`);
              continue;
            }
            if (students.some(s => s.password === password)) {
              errors.push(`Baris ${rowNum}: PASSWORD (${password}) sudah terdaftar di sistem.`);
              continue;
            }
          }
        ...
        if (errors.length > 0) {
          showToast(`Terdapat ${errors.length} baris yang gagal diimpor.`, "error");
        }
    ...
    classesWithData.forEach(className => {
      ...
      if (classResults.length > 0) {
        ...
        classResults.forEach(r => {
          const studentAvg = (Object.values(r.scores).reduce((a, b) => (a as number) + (typeof b === 'number' ? b : 0), 0) as number) / (Object.keys(r.scores).length || 1);
...
Berikan analisis mendalam mengenai karakteristik kelas ini berdasarkan hasil tes tersebut. ${selectedClass === 'all' ? 'Fokuskan pada tren kinerja antar kelas dan bandingkan performanya.' : ''} Apa kekuatan utama kelas ini? Apa area yang perlu mendapat perhatian lebih dari guru? Berikan rekomendasi strategi pembelajaran atau pendekatan bimbingan yang cocok untuk mayoritas siswa di kelas ini. Gunakan bahasa Indonesia yang profesional, mudah dipahami, dan format Markdown yang rapi. Sertakan ringkasan temuan utama dan saran yang dapat ditindaklanjuti.`;
  ...
  const mostFrequentTest = Object.entries(testTypeCounts).sort((a, b) => (b[1] as number) - (a[1] as number))[0];
  ...
  const avgScore = frequentTestResults.length > 0
    ? (frequentTestResults.reduce((acc, curr) => {
        const totalScore = Object.values(curr.scores).reduce((a, b) => (a as number) + (b as number), 0);
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden relative">
      ...
      >
        <div className="p-6 border-b border-emerald-700/50 bg-gradient-to-br from-emerald-500/20 to-transparent">
          <h2 className="text-2xl font-black tracking-tight text-emerald-50">PsikoTest</h2>
          ...
          <p className="text-[10px] text-emerald-200/60 mt-3 font-medium opacity-80">{user.email}</p>
        </div>
        <div className="flex-1 overflow-y-auto py-6 space-y-6 px-4">
          {['Utama', 'Manajemen Siswa', 'Manajemen Umum', 'Administrasi'].map(groupName => {
            const tabsInGroup = sidebarOrder.filter(id => AVAILABLE_TABS.find(t => t.id === id)?.group === groupName);
            if (tabsInGroup.length === 0) return null;
            
            const groupColor = groupName === 'Manajemen Umum' ? 'text-indigo-300/50' : 'text-emerald-300/50';
            
            return (
              <div key={groupName}>
                <p className={cn("text-[10px] font-black uppercase tracking-widest px-4 mb-2", groupColor)}>{groupName}</p>
                <div className="space-y-1">
                  {tabsInGroup.map(id => {
                    ...
                    return (
                      <button 
                        ...
                        className={cn(
                          ...
                          isActive 
                            ? (config.activeColorClass || "bg-emerald-600 text-white shadow-lg") 
                            : (id === 'dashboard' || id === 'teacher' || id === 'customization' ? "text-emerald-100/70 hover:bg-emerald-800/50 font-black" : (config.colorClass || "text-emerald-100/70 hover:bg-emerald-800/50"))
                        )}
                      >
        ...
        {(user.email?.toLowerCase() === "dutatama@gmail.com" || user.role === 'admin') && (
          <div className="px-4 pb-6">
            <button 
              ...
              className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-lg text-[11px] font-black transition-all tracking-wide", activeTab === 'admin-users' ? "bg-red-700 text-white shadow-lg" : "text-red-100/70 hover:bg-red-800/50")}
            >
      ...
      <div className="flex-1 min-w-0 overflow-y-auto p-4 sm:p-8 bg-slate-50/50">
        <div className="w-full space-y-8 px-2 sm:px-6">
          ...
          {notifications.filter(n => n.type === 'warning' && !n.read).length > 0 && (
            <motion.div 
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
            ...
            >
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h4 className="text-xl font-black text-red-900 leading-none">PERHATIAN KRITIS!</h4>
                  <p className="text-red-700 text-sm mt-2 font-bold leading-relaxed">
                    Terdapat {notifications.filter(n => n.type === 'warning' && !n.read).length} laporan kecemasan tinggi yang belum ditangani.
                    Mohon segera periksa notifikasi dan berikan bimbingan kepada siswa terkait.
                  </p>
                </div>
              </div>
              <div className="flex gap-3">
          ...
          <div className="bg-emerald-500 p-6 rounded-2xl border border-emerald-600 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-6">
              <h2 className="text-2xl font-black text-white tracking-tight">
                ...
                {activeTab === 'report-individu-umum' && 'Laporan Individu Umum'}
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {enabledWidgets.includes('test_distribution') && (
                  <div className={cn("bg-white p-6 rounded-2xl border border-slate-200 shadow-sm", enabledWidgets.includes('quick_stats') ? "lg:col-span-2" : "lg:col-span-3")}>
                ...
                {enabledWidgets.includes('quick_stats') && (
                  <div className={cn("bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col", !enabledWidgets.includes('test_distribution') && "lg:col-span-3")}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      <h3 className="text-xl font-black text-slate-900 flex items-center gap-3 uppercase tracking-tight">
                        <Layout className="w-6 h-6 text-emerald-600" /> Pilih Widget Dashboard
                      </h3>
                      <p className="text-sm text-slate-500 mt-1 font-medium">Aktifkan widget yang ingin Anda tampilkan di halaman utama dashboard.</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    {AVAILABLE_WIDGETS.map(widget => (
                      <label key={widget.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-emerald-200 transition-all cursor-pointer group">
                        <div className="flex items-center gap-4">
                          <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center transition-all", enabledWidgets.includes(widget.id) ? "bg-emerald-600 text-white shadow-lg" : "bg-white text-slate-400 border border-slate-200")}>
                            {enabledWidgets.includes(widget.id) ? <Check className="w-5 h-5" /> : <Settings2 className="w-5 h-5" />}
                          </div>
                          <span className={cn("font-bold text-sm", enabledWidgets.includes(widget.id) ? "text-slate-900" : "text-slate-500")}>{widget.name}</span>
                        </div>
                        <input 
                          type="checkbox" 
                          className="hidden" 
                          ...
                          onChange={(e) => {
                            if (e.target.checked) setEnabledWidgets([...enabledWidgets, widget.id]);
                            else setEnabledWidgets(enabledWidgets.filter(w => w !== widget.id));
                          }}
                        />
                        <div className={cn("w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all", enabledWidgets.includes(widget.id) ? "border-emerald-600 bg-emerald-600" : "border-slate-200 bg-white")}>
                          {enabledWidgets.includes(widget.id) && <Check className="w-4 h-4 text-white" />}
                        </div>
                      </label>
                    ))}
                  </div>
                ...
                <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      <h3 className="text-xl font-black text-slate-900 flex items-center gap-3 uppercase tracking-tight">
                        <GripVertical className="w-6 h-6 text-emerald-600" /> Urutan Sidebar & Tab
                      </h3>
                      <p className="text-sm text-slate-500 mt-1 font-medium">Gunakan panah untuk mengatur urutan menu navigasi di sidebar.</p>
                    </div>
                  </div>
                  <div className="space-y-2 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                    {sidebarOrder.map((tabId, index) => {
                      ...
                      return (
                        <div key={tabId} className="flex items-center justify-between p-3.5 bg-slate-50 rounded-2xl border border-slate-100 group hover:shadow-md transition-all">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="font-bold text-sm text-slate-800 tracking-tight">{tabInfo.name}</p>
                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{tabInfo.group}</p>
                            </div>
                          </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-wrap gap-4 items-center justify-between">
                  <div className="max-w-md">
                    <h3 className="text-lg font-bold text-slate-800">Daftar Pengguna & Hak Akses</h3>
                    <p className="text-sm text-slate-500 mt-1">
                      Guru BK lain harus login terlebih dahulu menggunakan Google agar akun mereka muncul di daftar ini. 
                      Setelah itu, Anda dapat mengubah role mereka menjadi 'Guru BK' atau 'Admin'.
                    </p>
                  </div>
                ...
                <div className="overflow-x-auto min-w-[1000px]">
                        ...
                        <tr key={u.id} className="hover:bg-slate-50/80 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className={cn(
                              ...
                              u.role === 'teacher' ? "bg-amber-100 text-amber-600" : "bg-emerald-100 text-emerald-600"
                            )}>
                              {u.role === 'admin' ? 'Administrator' : u.role === 'teacher' ? 'Guru BK' : 'Siswa'}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-right">
                            <div className="flex flex-col items-end gap-2">
                              <div className="flex items-center justify-end gap-2">
                                <select 
                                  disabled={u.email?.toLowerCase() === "dutatama@gmail.com"}
                                  value={u.role}
                                  onChange={async (e) => {
                                    try {
                                      await updateDoc(doc(db, 'users', u.id), { role: e.target.value });
                                      showToast(`Role ${u.name} berhasil diubah menjadi ${e.target.value}`, 'success');
                                    } catch (error) {
                                ...
                                <button
                                  onClick={() => setConfirmDelete({ id: u.id, type: 'user', title: 'Hapus Pengguna?', message: `Apakah Anda yakin ingin menghapus pengguna ${u.name}?` })}
                                  disabled={u.email?.toLowerCase() === "dutatama@gmail.com"}
                              ...
                              {u.role === 'teacher' && (
                                <div className="flex items-center gap-2">
                                  <div className={cn(
                                    ...
                                    u.expiryDate && new Date(u.expiryDate.seconds * 1000) < new Date() 
                                    ...
                                    <span className="uppercase tracking-tighter shrink-0 text-[9px]">Masa Aktif:</span>
                                    <input 
                                      type="date"
                                      value={u.expiryDate ? new Date(u.expiryDate.seconds * 1000).toISOString().split('T')[0] : ''}
                                      onChange={async (e) => {
                                        try {
                                          const date = new Date(e.target.value);
                                          date.setHours(23, 59, 59, 999);
                                          await updateDoc(doc(db, 'users', u.id), { expiryDate: Timestamp.fromDate(date) });
                                          showToast(`Masa aktif Guru BK ${u.name} berhasil diperbarui.`, 'success');
                                        } catch (error) {
                                          handleFirestoreError(error, OperationType.UPDATE, 'users');
                                        }
                                      }}
                                    ...
                                    />
                                    {u.expiryDate && new Date(u.expiryDate.seconds * 1000) < new Date() && (
                                      <span className="w-2 h-2 bg-red-600 rounded-full animate-pulse ml-1" title="Kadaluarsa" />
                                    )}
                                  </div>
                                </div>
                              )}
                  ...
                  <PaginationControls
                    currentPage={currentPage}
                    totalPages={Math.ceil(allUsers.filter(u => u.email && u.email.trim() !== '').filter(u => {
                      const matchesSearch = u.name.toLowerCase().includes(userSearch.toLowerCase()) || 
                                            u.email.toLowerCase().includes(userSearch.toLowerCase());
                      const matchesRole = userRoleFilter === 'all' || u.role === userRoleFilter;
                      return matchesSearch && matchesRole;
                    ...
                    onPageChange={setCurrentPage}
                    totalItems={allUsers.filter(u => u.email && u.email.trim() !== '').filter(u => {
                      const matchesSearch = u.name.toLowerCase().includes(userSearch.toLowerCase()) || 
                                            u.email.toLowerCase().includes(userSearch.toLowerCase());
                      const matchesRole = userRoleFilter === 'all' || u.role === userRoleFilter;
                      return matchesSearch && matchesRole;
          ...
          {activeTab === 'history' && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        isSelectingHistory ? "bg-slate-800 text-white border-slate-800" : "bg-white text-slate-700 border-slate-200 hover:bg-slate-50"
                      )}
                    >
              ...
              <div className="overflow-x-auto" style={{ transform: 'rotateX(180deg)' }}>
                <div style={{ transform: 'rotateX(180deg)' }}>
                    ...
                    <tr>
                      {isSelectingHistory && (
                        <th className="px-6 py-4 w-10">
                          <input
                            type="checkbox"
                            checked={selectedHistoryIds.length === paginatedResults.length && paginatedResults.length > 0}
                            onChange={toggleSelectAllHistory}
                            className="rounded border-slate-300 text-emerald-600 focus:ring-emerald-600 cursor-pointer"
                          />
                        </th>
                      )}
                      <th className="px-6 py-4 cursor-pointer hover:bg-slate-50 transition-colors group" onClick={() => toggleHistorySort('name')}>
                        <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest">
                          Siswa 
                          <ArrowUpDown className={cn("w-3 h-3 transition-opacity", historySortField === 'name' ? "opacity-100 text-emerald-600" : "opacity-20 group-hover:opacity-100")} />
                        </div>
                      </th>
                      <th className="px-6 py-4 cursor-pointer hover:bg-slate-50 transition-colors group" onClick={() => toggleHistorySort('class')}>
                        <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest">
                          Kelas 
                          <ArrowUpDown className={cn("w-3 h-3 transition-opacity", historySortField === 'class' ? "opacity-100 text-emerald-600" : "opacity-20 group-hover:opacity-100")} />
                        </div>
                      </th>
                      <th className="px-6 py-4 cursor-pointer hover:bg-slate-50 transition-colors group" onClick={() => toggleHistorySort('school')}>
                      ...
                      <th className="px-6 py-4 cursor-pointer hover:bg-slate-50 transition-colors group" onClick={() => toggleHistorySort('type')}>
                        <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest">
                          Jenis Tes 
                          <ArrowUpDown className={cn("w-3 h-3 transition-opacity", historySortField === 'type' ? "opacity-100 text-emerald-600" : "opacity-20 group-hover:opacity-100")} />
                        </div>
                      </th>
                      <th className="px-6 py-4 cursor-pointer hover:bg-slate-50 transition-colors group" onClick={() => toggleHistorySort('date')}>
                        <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest">
                          Tanggal 
                          <ArrowUpDown className={cn("w-3 h-3 transition-opacity", historySortField === 'date' ? "opacity-100 text-emerald-600" : "opacity-20 group-hover:opacity-100")} />
                        </div>
                      </th>
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {paginatedResults.map((r, i) => {
                      ...
                      return (
                      <tr key={r.id || i} className={cn("hover:bg-slate-50/80 transition-colors text-sm", selectedHistoryIds.includes(r.id!) ? "bg-emerald-50/50" : "")}>
                        {isSelectingHistory && (
                          <td className="px-6 py-4">
                            <input
                              type="checkbox"
                        ...
                        <td className="px-6 py-4 text-slate-600 font-medium">{student?.schoolName || '-'}</td>
                    ...
                    {tableFilteredResults.length === 0 && (
                      <tr>
                        <td colSpan={isSelectingHistory ? 10 : 9} className="px-6 py-12 text-center text-slate-400 text-sm font-medium">Tidak ada hasil yang cocok dengan filter.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
                <PaginationControls 
                  currentPage={currentPage}
                  totalPages={Math.ceil(tableFilteredResults.length / itemsPerPage)}
                  onPageChange={setCurrentPage}
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button 
                    onClick={() => handlePrintLoginCards(selectedStudentIds.length > 0 ? students.filter(s => selectedStudentIds.includes(s.id!)) : filteredStudents)}
                  ...
                  >
                    <Contact className="w-4 h-4" /> CETAK KARTU LOGIN {selectedStudentIds.length > 0 ? '(TERPILIH)' : ''}
                  </button>
                  <button 
                    onClick={() => setConfirmDelete({
                      id: 'all',
                      ...
                      message: 'Apakah Anda yakin ingin menghapus SEMUA data siswa? Tindakan ini akan menghapus SEMUA hasil tes dan catatan konseling terkait. Tindakan ini tidak dapat dibatalkan.'
                    })}
              ...
              <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <LayoutDashboard className="w-3 h-3" /> Ringkasan Siswa per Kelas
                </h4>
                <div className="flex flex-wrap gap-2">
                  {classes.map(c => (
                    <div key={c.id} className="bg-white px-3 py-2 rounded-xl border border-slate-200 flex items-center gap-3 shadow-sm hover:shadow-md transition-shadow">
                      <div className="w-8 h-8 bg-indigo-50 rounded-lg flex items-center justify-center shrink-0">
                        <Users className="w-4 h-4 text-indigo-600" />
                      </div>
                      <div>
                        <p className="text-[9px] font-black text-slate-400 leading-none mb-1 uppercase tracking-tighter">{c.name}</p>
                        <p className="text-xs font-bold text-slate-700 leading-none">{studentsCountByClass[c.name] || 0} Siswa</p>
                      </div>
                    </div>
                  ))}
                  {Object.entries(studentsCountByClass).filter(([cName]) => !classes.some(c => c.name === cName)).map(([cName, count]) => (
                    <div key={cName} className="bg-white px-3 py-2 rounded-xl border border-amber-200 flex items-center gap-3 shadow-sm">
                      <div className="w-8 h-8 bg-amber-50 rounded-lg flex items-center justify-center shrink-0">
                        <AlertTriangle className="w-4 h-4 text-amber-600" />
                      </div>
                      <div>
                        <p className="text-[9px] font-black text-amber-600 leading-none mb-1 uppercase tracking-tighter">{cName}</p>
                        <p className="text-xs font-bold text-slate-700 leading-none">{count} Siswa</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  <motion.div 
                    ...
                    className={cn(
                      ...
                      uploadSummary.success === uploadSummary.total ? "bg-emerald-50 border border-emerald-200 text-emerald-800" : "bg-amber-50 border border-amber-200 text-amber-800"
                    )}
                  >
                    ...
                    <span className="font-bold">
                      Selesai Impor: {uploadSummary.success} Berhasil, {uploadSummary.total - uploadSummary.success} Gagal (Total {uploadSummary.total} Baris).
                    </span>
                    <button onClick={() => setUploadSummary(null)} className="ml-auto text-[10px] font-black uppercase tracking-widest opacity-50 hover:opacity-100">Tutup</button>
                  </motion.div>
                  ...
                  {importErrors.length > 0 && (
                    ...
                    >
                      <div className="flex items-center gap-2 text-rose-800 font-bold text-xs mb-3 uppercase tracking-wider">
                        <AlertCircle className="w-4 h-4" /> Detail Kesalahan Impor ({importErrors.length})
                      </div>
                      <div className="max-h-32 overflow-y-auto space-y-1 pr-2 custom-scrollbar">
                        {importErrors.map((err, idx) => (
                          <div key={idx} className="text-[10px] font-medium text-rose-600 flex gap-2">
                    ...
                    <tr className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                      <th className="px-6 py-4 w-12">
                        <input 
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setSelectedStudentIds(filteredStudents.map(s => s.id!).filter(Boolean));
                            } else {
                              setSelectedStudentIds([]);
                            }
                          }}
                      ...
                      <th className="px-6 py-4">No</th>
                      <th className="px-6 py-4">PASSWORD</th>
                      <th className="px-6 py-4 cursor-pointer hover:bg-slate-100" onClick={() => { setSortStudentField('name'); setSortStudentOrder(sortStudentField === 'name' && sortStudentOrder === 'asc' ? 'desc' : 'asc'); }}>Nama Siswa {sortStudentField === 'name' && (sortStudentOrder === 'asc' ? '▲' : '▼')}</th>
                      <th className="px-6 py-4 cursor-pointer hover:bg-slate-100" onClick={() => { setSortStudentField('className'); setSortStudentOrder(sortStudentField === 'className' && sortStudentOrder === 'asc' ? 'desc' : 'asc'); }}>Kelas {sortStudentField === 'className' && (sortStudentOrder === 'asc' ? '▲' : '▼')}</th>
                      <th className="px-6 py-4 cursor-pointer hover:bg-slate-100" onClick={() => { setSortStudentField('schoolName'); setSortStudentOrder(sortStudentField === 'schoolName' && sortStudentOrder === 'asc' ? 'desc' : 'asc'); }}>Nama Sekolah {sortStudentField === 'schoolName' && (sortStudentOrder === 'asc' ? '▲' : '▼')}</th>
                      <th className="px-6 py-4 text-right">Aksi</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 bg-white">
                      ...
                      filteredStudents.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((s, idx) => (
                        <tr 
                          ...
                          className={cn(
                            ...
                            selectedStudentIds.includes(s.id!) ? "bg-emerald-50/30" : (recentlyAddedIds.includes(s.id!) ? "bg-emerald-50/50" : "hover:bg-slate-50/80")
                          )}
                        >
                          <td className="px-6 py-4">
                            <input 
                          ...
                          {editingStudent?.id === s.id ? (
                                ...
                                <div className="relative group/pwd">
                                  <input 
                                    ...
                                    onChange={(e) => {
                                      const val = e.target.value.toUpperCase();
                                      if (val === '' || /^[0-9A-Z]+$/.test(val)) {
                                        setEditingStudent({ ...editingStudent, password: val });
                                      }
                                    }}
                                  ...
                                  <button
                                    onClick={() => {
                                      ...
                                      setEditingStudent({ ...editingStudent, password: result });
                                    }}
                                    title="Generate Password Otomatis"
                            ...
                            <>
                              ...
                              <td className="px-6 py-4 font-mono text-xs text-slate-500">{s.password}</td>
                              <td className="px-6 py-4">
                                <div className="flex items-center gap-2">
                              ...
                              <td className="px-6 py-4 text-slate-600 font-medium">{s.schoolName || '-'}</td>
                              <td className="px-6 py-4 text-right">
                                <div className="flex justify-end gap-2">
                                  <button 
          ...
          {activeTab === 'teacher' && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm max-w-3xl space-y-6">
              ...
              <div className="pt-8 mt-8 border-t border-slate-200">
                ...
                <div className="pt-8 mt-8 border-t border-slate-200 w-full">
                  <h3 className="text-lg font-black text-slate-900 mb-2 flex items-center gap-2">
                    <Database className="w-5 h-5 text-emerald-600" /> BACKUP & RESTORE DATA
                  </h3>
                  <p className="text-sm text-slate-500 mb-6 font-medium leading-relaxed">
                    Ekspor seluruh data aplikasi (Siswa, Hasil Tes, Catatan Konseling, Pengaturan) ke dalam file JSON. 
                    Gunakan fitur ini untuk mencadangkan data ke HDD atau Flashdisk Anda secara berkala.
                  </p>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-6 bg-emerald-50 rounded-2xl border border-emerald-100 flex flex-col justify-between">
                      <div>
                        <h4 className="font-black text-emerald-900 text-sm uppercase mb-2">Cadangkan Data (Backup)</h4>
                        <p className="text-xs text-emerald-700 mb-4 font-medium">Download semua data sistem saat ini untuk disimpan secara lokal.</p>
                      </div>
                      <button 
                    ...
                    <div className="p-6 bg-slate-50 rounded-2xl border border-slate-200 flex flex-col justify-between">
                      <div>
                        <h4 className="font-black text-slate-900 text-sm uppercase mb-2">Pulihkan Data (Restore)</h4>
                        <p className="text-xs text-slate-500 mb-4 font-medium">Impor file backup (.json) untuk memulihkan data sistem.</p>
                      </div>
          ...
          {activeTab === 'report' && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                  <h3 className="text-lg font-black text-slate-900">Laporan Agregat: {selectedClass === 'all' ? 'Semua Kelas' : selectedClass}</h3>
                  <p className="text-xs text-slate-500 mt-0.5 font-medium">{filteredResults.filter(r => filterType === 'all' || r.testType === filterType).length} Data Tersedia</p>
                </div>
              ...
              {filterType === 'all' ? (
                <div className="py-12 text-center bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                  ...
                  <h4 className="text-lg font-black text-slate-800 mb-1">Pilih Jenis Tes</h4>
                  <p className="text-slate-500 text-xs font-medium max-w-xs mx-auto">Silakan pilih jenis tes pada filter di atas untuk melihat visualisasi data agregat kelas.</p>
                </div>
                ...
                <div className="py-12 text-center bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                  ...
                  <h4 className="text-lg font-black text-slate-700 mb-1">Data Belum Tersedia</h4>
                  <p className="text-slate-500 text-xs font-medium max-w-xs mx-auto">Belum ada siswa dari kelas ini yang menyelesaikan tes {TESTS[filterType as TestType]?.title || filterType}.</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {/* Quick Stats Row */}
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Partisipan</p>
                        <h5 className="text-lg font-black text-slate-900">{filteredResults.filter(r => r.testType === filterType).length} Siswa</h5>
                      </div>
                    </div>
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3">
                      <div className="w-10 h-10 bg-emerald-50 rounded-xl flex items-center justify-center shrink-0">
                      ...
                      <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Dominan</p>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex items-center justify-between mb-6">
                        <h4 className="font-black text-slate-800 text-base tracking-tight">Rata-rata Skor per Kelas</h4>
                      </div>
                        ...
                        <table className="w-full text-xs text-left">
                          ...
                          <tbody className="divide-y divide-slate-100">
                            {classAverages && classAverages.map((c) => {
                              ...
                              const classCombinedAvg = avgValues.length > 0
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex items-center justify-between mb-6">
                        <h4 className="font-black text-slate-800 text-base tracking-tight">Distribusi Rata-rata Skor</h4>
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex items-center justify-between mb-6">
                        <h4 className="font-black text-slate-800 text-base tracking-tight">Profil Kemampuan (Radar)</h4>
                        <div className="p-1.5 bg-slate-50 rounded-lg">
                      ...
                      <div className="h-64 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                          <RadarChart cx="50%" cy="50%" outerRadius="70%" data={classChartData}>
                            <PolarGrid stroke="#e2e8f0" />
                            <PolarAngleAxis dataKey="name" fontSize={9} tick={{fill: '#64748b', fontWeight: 600}} />
                  ...
                  {classComparisonData.length > 1 && (
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex items-center justify-between mb-6">
                        <div>
                          <h4 className="font-black text-slate-800 text-base tracking-tight">Perbandingan Performa Kelas</h4>
                          <p className="text-[10px] text-slate-500 font-medium mt-0.5">Rata-rata skor keseluruhan untuk tes {TESTS[filterType as TestType]?.title}</p>
                        </div>
                    ...
                    <div className="bg-gradient-to-r from-emerald-600 to-teal-600 p-6 text-white">
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h4 className="text-lg font-black">Analisis Kelas Cerdas</h4>
                            <p className="text-emerald-100 text-xs font-medium">Wawasan mendalam berbasis AI untuk strategi bimbingan.</p>
                          </div>
                        </div>
                        {!classAnalysis && (
                          <button
                      ...
                      <div className="p-6 bg-slate-50/50">
                        ...
                        >
                          ...
                          <div className="mt-6 pt-6 border-t border-slate-100 flex justify-between items-center">
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Analisis dihasilkan secara otomatis oleh Gemini AI</p>
                            <button 
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div className="flex flex-wrap gap-3">
                        ...
                        <button 
                          onClick={() => setConfirmDelete({
                            ...
                            message: 'Apakah Anda yakin ingin menghapus SEMUA riwayat tes peserta umum? Tindakan ini tidak dapat dibatalkan.'
                          })}
                ...
                <div className="overflow-x-auto" style={{ transform: 'rotateX(180deg)' }}>
                    ...
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200">
                        <th className="p-4 text-center">
                          <input 
                            type="checkbox" 
                            checked={filteredRiwayatTamuResults.length > 0 && selectedResultIds.length === filteredRiwayatTamuResults.length}
                            onChange={toggleSelectRiwayatAll}
                            className="w-4 h-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-600 cursor-pointer"
                          />
                        </th>
                        <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-wider text-center">NO</th>
                        <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors group" onClick={() => toggleTamuSort('date')}>
                          <div className="flex items-center gap-1">
                            TANGGAL
                            <ArrowUpDown className={cn("w-3 h-3 transition-opacity", tamuSortField === 'date' ? "opacity-100 text-emerald-600" : "opacity-20 group-hover:opacity-100")} />
                          </div>
                        </th>
                        <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-wider">WAKTU</th>
                        <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors group" onClick={() => toggleTamuSort('name')}>
                          <div className="flex items-center gap-1">
                            NAMA SISWA
                            <ArrowUpDown className={cn("w-3 h-3 transition-opacity", tamuSortField === 'name' ? "opacity-100 text-emerald-600" : "opacity-20 group-hover:opacity-100")} />
                          </div>
                        </th>
                        <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-wider">KELAS</th>
                        <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors group" onClick={() => toggleTamuSort('school')}>
                          <div className="flex items-center gap-1">
                            NAMA SEKOLAH/ ALAMAT
                            <ArrowUpDown className="w-3 h-3" />
                          </div>
                        </th>
                        <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-wider">KELAS/ UMUR</th>
                        <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors group" onClick={() => toggleTamuSort('type')}>
                          <div className="flex items-center gap-1">
                            JENIS TES
                            <ArrowUpDown className={cn("w-3 h-3 transition-opacity", tamuSortField === 'type' ? "opacity-100 text-emerald-600" : "opacity-20 group-hover:opacity-100")} />
                          </div>
                        </th>
                        <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-wider">HASIL/SKOR</th>
                        <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-wider text-right">AKSI</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {filteredRiwayatTamuResults.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((result, idx) => (
                        <tr key={result.id} className={`hover:bg-slate-50/80 transition-colors ${selectedResultIds.includes(result.id!) ? 'bg-emerald-50/30' : ''}`}>
                          <td className="p-4 text-center">
                            <input 
                              type="checkbox" 
                              checked={selectedResultIds.includes(result.id!)}
                              onChange={() => toggleSelectRiwayat(result.id!)}
                              className="w-4 h-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-600 cursor-pointer"
                            />
                          </td>
                          ...
                          <td className="p-4 text-xs font-bold text-slate-600">
                            {result.timestamp?.seconds ? new Date(result.timestamp.seconds * 1000).toLocaleDateString('id-ID') : '-'}
                          </td>
                          <td className="p-4 text-xs font-medium text-slate-600">
                            {result.timestamp?.seconds ? new Date(result.timestamp.seconds * 1000).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : '-'}
                          </td>
                          <td className="p-4">
                            <div className="font-bold text-slate-900 leading-tight">{result.studentName}</div>
                            <div className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">{result.studentEmail || '-'}</div>
                          </td>
                          <td className="p-4">
                            <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded-md text-[9px] font-black border border-emerald-100 uppercase tracking-tighter">
                              {result.studentJenjang || allUsers.find(u => u.uid === result.studentId)?.jenjang || 'UMUM'}
                            </span>
                          </td>
                          <td className="p-4">
                            <div className="text-xs font-bold text-slate-700 leading-tight">{result.studentSchoolName || result.studentPassword || '-'}</div>
                          </td>
                          <td className="p-4">
                            <span className="text-[9px] font-black text-slate-700 px-1.5 py-0.5 bg-slate-100 rounded border border-slate-200 uppercase tracking-tighter">{result.studentClass || '-'}</span>
                          </td>
                          <td className="p-4">
                            <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-100">
                              {TESTS[result.testType]?.title || customTests.find(ct => ct.id === result.testType)?.title || result.testType}
                            </span>
                          </td>
                          <td className="p-4">
                  ...
                  <div className="p-4 border-t border-slate-200">
                    <PaginationControls 
                      currentPage={currentPage}
                      totalPages={Math.ceil(filteredRiwayatTamuResults.length / itemsPerPage)}
                      onPageChange={setCurrentPage}
      ...
      {confirmDelete && (
        <ConfirmModal 
          ...
          onConfirm={async () => {
            if (confirmDelete.type === 'student') await handleDeleteStudent(confirmDelete.id);
            else if (confirmDelete.type === 'guest') await handleDeleteGuest(confirmDelete.id);
            else if (confirmDelete.type === 'result') await handleDeleteResult(confirmDelete.id);
            else if (confirmDelete.type === 'all_students') await handleDeleteAllStudents();
            else if (confirmDelete.type === 'all_selected_students') await handleDeleteSelectedStudents();
            else if (confirmDelete.type === 'results_by_student') await handleDeleteAllResultsForStudent(confirmDelete.id, confirmDelete.extraData.name, confirmDelete.extraData.className);
            else if (confirmDelete.type === 'results_by_guest') await handleDeleteAllResultsForGuest(confirmDelete.id, confirmDelete.extraData.name);
            else if (confirmDelete.type === 'results_by_selected_guests') await handleDeleteSelectedGuests();
            else if (confirmDelete.type === 'all_selected_results') await handleDeleteSelectedResults();
            else if (confirmDelete.type === 'all_selected_history_results') await handleDeleteSelectedHistoryResults();
            else if (confirmDelete.type === 'all_guests_comprehensive') await handleDeleteAllGuestsResults();
            else if (confirmDelete.type === 'user') await handleDeleteUser(confirmDelete.id);
          }}
        />
      )}
          ...
          >
            ...
            <div className="p-8 border-b border-slate-100 flex justify-between items-start bg-gradient-to-r from-slate-50 to-white">
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h2 className="text-3xl font-black text-slate-900 tracking-tight">{viewingStudentProfile.name}</h2>
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <span className="text-slate-400 font-bold text-sm">PASSWORD: {viewingStudentProfile.password || '-'}</span>
                  </div>
                </div>
              </div>
              <button 
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                      <h3 className="text-lg font-black text-slate-900 flex items-center gap-3 uppercase tracking-tight">
                        <TrendingUp className="w-5 h-5 text-emerald-600" /> Analisis Tren & Longitudinal
                      </h3>
                    </div>
                    
                    <div className="h-64 w-full">
                      {results.filter(r => r.studentId === viewingStudentProfile.id).length > 0 ? (
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={results
                            .filter(r => r.studentId === viewingStudentProfile.id)
                            .sort((a, b) => (a.timestamp?.seconds || 0) - (b.timestamp?.seconds || 0))
                            .map(r => ({
                              date: new Date(r.timestamp?.seconds * 1000).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }),
                              score: (Object.values(r.scores).reduce((a, b) => (a as number) + (b as number), 0) as number) / (Object.values(r.scores).length || 1),
                              test: TESTS[r.testType]?.title || r.testType
                            }))
                          }>
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                            <XAxis dataKey="date" fontSize={10} tick={{fill: '#94a3b8', fontWeight: 600}} axisLine={false} tickLine={false} />
                            <YAxis fontSize={10} tick={{fill: '#94a3b8', fontWeight: 600}} axisLine={false} tickLine={false} domain={[0, 100]} />
                            <Tooltip 
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-slate-400 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                          ...
                          <p className="text-sm font-bold">Belum ada data tes untuk dianalisis</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
                    <h3 className="text-lg font-black text-slate-900 mb-6 flex items-center gap-3 uppercase tracking-tight">
                      <History className="w-5 h-5 text-emerald-600" /> Riwayat Tes Lengkap
                    </h3>
                    <div className="space-y-4">
                      {results
                        .filter(r => r.studentId === viewingStudentProfile.id)
                        ...
                        .map(result => (
                          <div key={result.id} className="flex items-center justify-between p-5 bg-slate-50 rounded-2xl border border-slate-100 hover:border-emerald-200 transition-all group">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <p className="font-black text-slate-900 text-sm">{TESTS[result.testType]?.title || result.testType}</p>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">
                                  {new Date(result.timestamp?.seconds * 1000).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                                </p>
                              </div>
                            </div>
                            <div className="flex items-center gap-3">
                      ...
                      {results.filter(r => r.studentId === viewingStudentProfile.id).length === 0 && (
                        <p className="text-center py-8 text-slate-400 font-medium text-sm italic">Belum ada riwayat tes.</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm h-full flex flex-col">
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-lg font-black text-slate-900 flex items-center gap-3 uppercase tracking-tight">
                        <MessageSquare className="w-5 h-5 text-emerald-600" /> Catatan Konseling
                      </h3>
                      <button 
                      ...
                      >
                        ...
                        <span className="text-[10px] font-black uppercase tracking-wider">Tambah Catatan</span>
                      </button>
                    </div>
                    ...
                    {isAddingLog && (
                      ...
                      >
                        <div className="flex justify-between items-center mb-2">
                          <h4 className="text-xs font-black text-emerald-800 uppercase tracking-widest">Sesi Konseling Baru</h4>
                          <button onClick={() => setIsAddingLog(false)} className="text-emerald-400 hover:text-emerald-600">
                            <X className="w-4 h-4" />
                          </button>
                        </div>
                        <input 
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input 
                              ...
                              onChange={(e) => setNewLogIsPrivate(e.target.checked)}
                              className="w-4 h-4 rounded border-emerald-300 text-emerald-600 focus:ring-emerald-500"
                            />
                            <span className="text-[10px] font-black text-emerald-800 uppercase tracking-wider">Privat</span>
                          </label>
                        </div>
                        <button 
                    ...
                    <div className="space-y-4 flex-1 overflow-y-auto pr-2">
                      {counselingLogs
                        .filter(log => log.studentId === viewingStudentProfile.id)
                        .sort((a, b) => (b.timestamp?.seconds || 0) - (a.timestamp?.seconds || 0))
                        .map(log => (
                          <div key={log.id} className="p-5 bg-slate-50 rounded-2xl border border-slate-100 relative group">
                            <div className="flex justify-between items-start mb-3">
                              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                {new Date(log.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                              </span>
                              <div className="flex items-center gap-2">
                                {log.isPrivate && <Lock className="w-3 h-3 text-amber-500" />}
                                <span className={cn(
                                  "px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-tighter",
                                  log.interventionStatus === 'completed' ? "bg-emerald-100 text-emerald-700" :
                                  log.interventionStatus === 'in-progress' ? "bg-blue-100 text-blue-700" :
                                  "bg-slate-200 text-slate-600"
                                )}>
                                  {log.interventionStatus === 'completed' ? 'Selesai' : 
                                   log.interventionStatus === 'in-progress' ? 'Proses' : 'Pending'}
                                </span>
                              </div>
                            </div>
                            <h4 className="font-black text-slate-900 text-sm mb-2">{log.topic}</h4>
                            <p className="text-xs text-slate-600 leading-relaxed line-clamp-3">{log.notes}</p>
                            
                            <button 
                              onClick={() => handleDeleteCounselingLog(log.id!)}
                              className="absolute top-4 right-4 p-1.5 text-red-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"
                            >
                              <Trash2 className="w-3.5 h-3.5" />
                            </button>
                          </div>
                        ))}
                      {counselingLogs.filter(log => log.studentId === viewingStudentProfile.id).length === 0 && (
                        <div className="flex flex-col items-center justify-center py-12 text-slate-300">
                          <MessageSquare className="w-12 h-12 mb-3 opacity-20" />
                          <p className="text-xs font-bold uppercase tracking-widest">Belum ada catatan</p>
                        </div>
                      )}
                    </div>
                  </div>
...
const IdentityForm = ({ classes, students, onSave, onLogout, initialStep = 'initial' }: { classes: ClassInfo[], students: StudentData[], onSave: (data: { name: string, className: string, password?: string, schoolName?: string, jenjang?: string }) => void, onLogout: () => void, initialStep?: 'initial' | 'registered' | 'studentCard' | 'guest' }) => {
  ...
  const handleCheckPassword = () => {
    ...
    const student = students.find(s => s.password?.trim().toUpperCase() === trimmedInput);
    if (student) {
  ...
  const handleGuestSubmit = async () => {
    ...
    if (!guestAsal.trim()) newErrors.asal = 'Asal sekolah atau alamat wajib diisi.';
  ...
  const handleStudentSubmit = async () => {
    if (foundStudent) {
      ...
      setErrorMsg('');
      try {
        await onSave({ name: foundStudent.name, className: foundStudent.className, password: foundStudent.password });
      } catch (error: any) {
        let msg = "Terjadi kesalahan saat menyimpan data.";
        try {
          const parsed = JSON.parse(error.message);
      ...
      >
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100">
              <p className="text-[11px] text-amber-800 leading-relaxed">
                <span className="font-bold">Petunjuk :</span> Siswa yang terdaftar sebagai siswa bimbingan, silahkan pilih tombol <span className="font-bold">SISWA TERDAFTAR</span> (Login dengan menggunakan Password). Apabila belum terdaftar silahkan hubungi guru BK, bagi peserta tes yang lain silahkan pilih tombol <span className="font-bold">UMUM</span>.
              </p>
            </div>
        ...
        {step === 'registered' && (
          <div className="space-y-6">
            ...
            <p className="text-slate-500 text-sm">Masukkan Password Anda (apabila belum punya, tanyakan guru BK anda)</p>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-emerald-50 p-6 rounded-3xl border border-emerald-100 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Nama Sekolah</p>
                  <p className="text-base font-bold text-slate-800">{foundStudent.schoolName || '-'}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">PASSWORD</p>
                  <div className="flex items-center gap-2">
        ...
        {step === 'guest' && (
          <div className="space-y-6">
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">Pendaftaran Peserta</h2>
            <p className="text-slate-500 text-sm font-medium">Lengkapi data diri untuk memulai tes psikologi sebagai peserta umum.</p>
            
            <div className="space-y-5">
              <div className="space-y-1.5">
                <label className="block text-xs font-black text-slate-500 uppercase tracking-widest">Nama Lengkap</label>
                <div className="relative">
                  <UserIcon className={cn("absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors", formErrors.name ? "text-red-400" : "text-slate-400")} />
                  <input 
                    type="text"
                    value={guestName}
                    onChange={(e) => {
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-black text-slate-500 uppercase tracking-widest">Jenjang Pendidikan</label>
                <div className="relative">
                  <GraduationCap className={cn("absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors z-10", formErrors.jenjang ? "text-red-400" : "text-slate-400")} />
                  <select 
                    value={guestJenjang}
                    onChange={(e) => {
                      setGuestJenjang(e.target.value);
                      if (formErrors.jenjang) setFormErrors(prev => ({ ...prev, jenjang: '' }));
                    }}
                    className={cn(
                  ...
                  >
                    ...
                    {['SMP', 'MTs', 'SMA', 'SMK', 'KULIAH', 'UMUM'].map(j => <option key={j} value={j}>{j}</option>)}
                  </select>
                </div>
                {formErrors.jenjang && <p className="text-[10px] font-bold text-red-500 pl-1">{formErrors.jenjang}</p>}
              </div>

              <div className="space-y-1.5">
                <label className="block text-xs font-black text-slate-500 uppercase tracking-widest">Asal Sekolah / Instansi</label>
                <div className="relative">
                  <Home className={cn("absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors", formErrors.asal ? "text-red-400" : "text-slate-400")} />
                  <input 
                    type="text"
                    value={guestAsal}
                    onChange={(e) => {
                      setGuestAsal(e.target.value);
                      if (formErrors.asal) setFormErrors(prev => ({ ...prev, asal: '' }));
                    }}
                ...
                {formErrors.asal && <p className="text-[10px] font-bold text-red-500 pl-1">{formErrors.asal}</p>}
              </div>

              <div className="space-y-1.5">
                <label className="block text-xs font-black text-slate-500 uppercase tracking-widest">Kelas / Umur</label>
                <div className="relative">
                  <Hash className={cn("absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors", formErrors.kelas ? "text-red-400" : "text-slate-400")} />
                  <input 
                    type="text"
                    value={guestKelas}
                    onChange={(e) => {
                      setGuestKelas(e.target.value);
                      if (formErrors.kelas) setFormErrors(prev => ({ ...prev, kelas: '' }));
                    }}
                ...
                {formErrors.kelas && <p className="text-[10px] font-bold text-red-500 pl-1">{formErrors.kelas}</p>}
              </div>
            </div>
      ...
      <div className="relative z-10 max-w-4xl w-full px-6 text-center space-y-12">
        ...
        >
          ...
          <p className="text-lg md:text-2xl text-slate-600 font-medium max-w-2xl mx-auto leading-relaxed italic font-serif">
            "Jelajahi potensi diri, pahami pola pikir, dan temukan jalan menuju pertumbuhan optimal dengan akurasi psikologis."
          </p>
        </motion.div>
...
export default function App() {
  ...
  const [view, setView] = useState<'dashboard' | 'admin' | 'guide' | 'create-test' | 'history'>('dashboard');
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      try {
        if (firebaseUser) {
          const isAdminEmail = firebaseUser.email?.toLowerCase() === "dutatama@gmail.com";
          ...
          if (userDoc.exists()) {
            ...
            const isExpired = userData.expiryDate && new Date(userData.expiryDate.seconds * 1000) < new Date();
            
            if (isBKTeacher && isExpired && !isAdminEmail) {
              showToast(`Masa aktif akun Guru BK Anda telah berakhir. Silakan hubungi Admin.`, 'error');
              setUser(userData);
              setView('dashboard');
              ...
              setView('admin');
            } else if (!isAdminEmail && firebaseUser.email?.toLowerCase() === "purnomowiwit@gmail.com" && userData.role === 'admin') {
              await updateDoc(doc(db, 'users', firebaseUser.uid), { role: 'teacher' });
  ...
  const handleUpdateProfile = async (data?: { name: string, className: string, password?: string, schoolName?: string, jenjang?: string }) => {
    const finalData = data || { name: profileData.name, className: profileData.className, password: '', schoolName: '', jenjang: '' };
    ...
    try {
      ...
      try {
        ...
        if (finalData.password) {
          ...
          q = query(studentsRef, where('name', '==', finalData.name), where('className', '==', finalData.className));
        }
  ...
  useEffect(() => {
    if (user) {
      ...
      const isSuperAdmin = user.role === 'admin' || isAdminEmail;
      const isLegacyAdmin = user.email?.toLowerCase() === "purnomowiwit@gmail.com";

      const studentsQuery = (isSuperAdmin || user.role === 'student')
        ? collection(db, 'students')
        : isLegacyAdmin
          ? query(collection(db, 'students'), where('addedBy', 'in', [user.uid, 'admin']))
          : query(collection(db, 'students'), where('addedBy', '==', user.uid));
      ...
      const customTestsQuery = user.role === 'student' 
        ? query(collection(db, 'custom_tests'), where('teacherId', '==', (user as any).teacherId || 'admin'))
        : isSuperAdmin
          ? collection(db, 'custom_tests')
          : isLegacyAdmin
            ? query(collection(db, 'custom_tests'), where('teacherId', 'in', [user.uid, 'admin']))
            : query(collection(db, 'custom_tests'), where('teacherId', '==', user.uid));
      ...
      const qNotif = user.role === 'student'
        ? query(
      ...
      if (user.role === 'teacher' || user.role === 'admin') {
        const qLogs = isSuperAdmin
          ...
          : isLegacyAdmin
            ? query(collection(db, 'counseling_logs'), where('teacherId', 'in', [user.uid, 'admin']))
            : query(collection(db, 'counseling_logs'), where('teacherId', '==', user.uid));
        ...
        const qLogs = query(collection(db, 'counseling_logs'), where('studentId', '==', user.uid), where('isPrivate', '==', false));
        unsubLogs = onSnapshot(qLogs, (snapshot) => {
          setAllCounselingLogs(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as CounselingLog)));
        });
      }
      ...
      if (user.role === 'teacher' || user.role === 'admin') {
        const qAll = isSuperAdmin
          ? query(collection(db, 'test_results'), orderBy('timestamp', 'desc'))
          : isLegacyAdmin
            ? query(collection(db, 'test_results'), where('teacherId', 'in', [user.uid, 'admin']), orderBy('timestamp', 'desc'))
            : query(collection(db, 'test_results'), where('teacherId', '==', user.uid), orderBy('timestamp', 'desc'));
  ...
  const getRecommendation = (type: TestType) => {
    ...
    if (type === 'subject_interest') {
      if (latest.analysis.includes('Sains')) return "Perdalam pemahaman konsep matematika dan sains.";
      if (latest.analysis.includes('Sastra')) return "Tingkatkan kemampuan bahasa dan ekspresi kreatif.";
      return "Eksplorasi materi pelajaran yang relevan dengan minat Anda.";
    }
    if (type === 'multiple_intelligences') {
      if (latest.analysis.includes('Logis')) return "Asah kemampuan problem solving dan logika.";
      if (latest.analysis.includes('Linguistik')) return "Tingkatkan kosakata dan kemampuan menulis.";
...
${testType === 'subject_interest' && extraData?.reasons ? `Minat Mata Pelajaran & Alasan:\n${Object.entries(extraData.reasons).map(([id, reason]) => {
  const map: Record<string, string> = {
    agama: "Pendidikan Agama", ppkn: "PPKn", b_indo: "Bahasa Indonesia",
    mtk: "Matematika", ipa: "IPA", ips: "IPS", b_ing: "Bahasa Inggris",
    seni: "Seni Budaya", pjok: "PJOK", prakarya: "Prakarya",
          ...
          >
            ...
            <div className="mt-8 p-5 bg-indigo-50 rounded-2xl border border-indigo-100 text-left">
              <p className="text-[11px] text-indigo-900 leading-relaxed">
                <span className="font-black text-indigo-700">Petunjuk :</span> Siswa yang terdaftar sebagai siswa bimbingan, silahkan pilih tombol <b>SISWA TERDAFTAR</b> (Login dengan menggunakan Password). Apabila belum terdaftar silahkan hubungi guru BK, bagi peserta tes yang lain silahkan pilih tombol <b>UMUM</b>.
              </p>
            </div>
          ...
          {activeTest || activeCustomTest ? (
            ...
            >
              <ResultView 
                ...
                showToast={showToast} 
                teacherSettings={teacherSettings} 
                results={user?.role === 'admin' || user?.role === 'teacher' ? allResults : userResults} 
              />
            </motion.div>
          ) : view === 'admin' ? (
            <motion.div
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
            >
              <TestCreator onBack={() => setView('admin')} showToast={showToast} user={user!} />
            </motion.div>
          ) : view === 'guide' ? (
            <motion.div
              key="guide-view"
            ...
            >
              <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
                <div className="p-8 border-b border-slate-100 bg-emerald-50/30 flex items-center justify-between">
                  <div>
                    <h3 className="text-2xl font-black text-slate-900">Riwayat Tes Saya</h3>
                    <p className="text-slate-500 text-sm font-medium">Lihat dan unduh laporan hasil tes yang telah kamu selesaikan.</p>
                  </div>
                  <button 
                  ...
                  {userResults.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {userResults.sort((a,b) => {
                        ...
                        <div key={i} className="group p-6 rounded-3xl border border-slate-100 bg-slate-50/30 hover:bg-white hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-500/5 transition-all flex flex-col gap-5">
                           <div className="flex items-start justify-between">
                             <div className="flex items-center gap-4">
                               ...
                               <div>
                                 <h4 className="font-black text-slate-900 text-sm leading-tight mb-1">{TESTS[res.testType]?.title || res.testType}</h4>
                                 <p className="text-[10px] text-emerald-600 font-black uppercase tracking-widest bg-emerald-50 px-2 py-0.5 rounded-md inline-block">
                                   {res.timestamp ? new Date(res.timestamp.seconds * 1000).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) : '-'}
                                 </p>
                               </div>
                             </div>
                           </div>
                    ...
                    <div className="py-24 text-center">
                       ...
                       <h4 className="text-lg font-black text-slate-900 mb-2">Belum ada riwayat tes</h4>
                       <p className="text-slate-500 text-sm font-medium mb-8">Kamu belum menyelesaikan tes apapun secara mandiri.</p>
                       <button 
                         onClick={() => setView('dashboard')}
            ...
            >
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
                ...
                {(user.role === 'teacher' || user.role === 'admin' || user.email.toLowerCase() === "dutatama@gmail.com") && (
                  <div className="flex gap-3">
                    <button 
              ...
              {resumeTestType && !activeTest && !activeCustomTest && (
                ...
                >
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <h4 className="text-lg font-black text-emerald-900">Progres Tersimpan</h4>
                      <p className="text-emerald-700 text-sm font-medium">Kami menemukan progres pengerjaan tes <b>{TESTS[resumeTestType]?.title || (customTests.find(ct => ct.testType === resumeTestType)?.title || resumeTestType)}</b> yang belum selesai.</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => {
                      const ctFound = customTests.find(ct => ct.testType === resumeTestType);
                      if (ctFound) {
